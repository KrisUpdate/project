...
int main(){
	...
	int rev_data_pass_cnt=0;
	int rev_data_fail_cnt=0;
	...
	while (1) {
		...
		printf("%s\n", buf);
		int check_data_result=0;
		for(int i=18;i<nbytes;i++){
			if(buf[i]!=(i-17)){
				check_data_result=1;		
			}
		}
		...
		printf("%d,%d \r\n",rev_data_pass_cnt,rev_data_fail_cnt);
		printf("Total Pass should rev %d form \r\n",rev_data_fail_cnt*63);
