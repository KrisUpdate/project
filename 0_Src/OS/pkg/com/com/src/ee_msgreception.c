...
#ifndef __PRIVATE_COM_QUEUE_FULL__
StatusType EE_com_queue_full (MessageIdentifier Message)
{
  const struct EE_com_msg_ROM_TYPE * msg_ROM;
  if (Message != EE_COM_NULL)
  {
...
#ifndef __PRIVATE_COM_QUEUE_LEFT__
StatusType EE_com_queue_left (MessageIdentifier Message)
{
  const struct EE_com_msg_ROM_TYPE * msg_ROM;
  msg_ROM = EE_com_msg_ROM[Message];

  if (Message != EE_COM_NULL)
    return (((struct EE_COM_msg_queue *)msg_ROM->data)->max - ((struct EE_COM_msg_queue *)msg_ROM->data)->tot);
  else
         return ((struct EE_COM_msg_queue *)msg_ROM->data)->max;
}
#endif
  ...
  {  
    ...
    { /* If the filter don't stop, i have to store new data 
      ...
      if (((struct EE_COM_msg_queue *)msg_ROM->data)->tot ==
      	((struct EE_COM_msg_queue *)msg_ROM->data)->max) {
        /* Full queue */
        ((struct EE_COM_msg_queue *)msg_ROM->data)->tot--;
        ((struct EE_COM_msg_queue *)msg_ROM->data)->first++;
        if (((struct EE_COM_msg_queue *)msg_ROM->data)->first ==
        	 ((struct EE_COM_msg_queue *)msg_ROM->data)->max) {
            ((struct EE_COM_msg_queue *)msg_ROM->data)->first = 0;
        }
      }
