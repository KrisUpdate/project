...
StatusType EE_com_check_queue (MessageIdentifier Message)
{
  ...
  {
    return EE_com_queue_full(msg_ROM->next);
  }
}
...
StatusType EE_com_queue_rem (MessageIdentifier Message)
{
  ...
  if (msg_ROM->next != EE_COM_NULL)
  {
    return EE_com_queue_left(msg_ROM->next);
  }
}
 ...
 */
StatusType EE_com_send (MessageIdentifier Message, EE_UINT8 *DataRef, EE_UINT16 size)
{  
  register const struct EE_com_msg_ROM_TYPE *msg_ROM = 
        EE_com_msg_ROM[Message];
