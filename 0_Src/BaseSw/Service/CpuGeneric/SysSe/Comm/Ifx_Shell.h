...
#define IFX_SHELL_COMMAND_LIST_END     {0, 0, 0, 0, 0}   // add a new uint16 for user type


#else

// There are duplicate declaration of "SHELL_HELP_DESCRIPTION_TEXT" in file "AsclinShellInterface.c"
...
#if (PROJECT == PROJECT_PAGASUS2)
// Menu item(= command) property
typedef struct _T_Menu_Item_Property_Detail
{
    uint16 User_Lvl               : 2 ;    // low bit first, User level, lower value means higher level
    uint16 Rev0                   : 14;
}T_MENU_ITEM_PROPERTY_DETAIL;

typedef union _T_Menu_Item_Property_Data
{
    uint8                         uB[2];
    uint16                        uW;
    T_MENU_ITEM_PROPERTY_DETAIL   detail;
}T_MENU_ITEM_PROPERTY_DATA;
