...
boolean Ifx_Shell_showHelpSingle(Ifx_Shell_CommandListConst commandList, IfxStdIf_DPipe *io, boolean briefOnly, boolean singleCommand)
{
    ...
    pchar                    space            = "";
    pchar                    spaceParam       = "";
    T_MENU_ITEM_PROPERTY_DATA     mproperty;

    while (command->commandLine != NULL_PTR)
    {
        mproperty.uW= command->property;
        //IfxStdIf_DPipe_print(io, "menu property= 0x%02X, user type= 0x%02X"ENDL, mproperty.uW, g_AsclinShellInterface.user_type);

        if(mproperty.detail.User_Lvl >= g_AsclinShellInterface.user_type)
        {
