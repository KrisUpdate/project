...
(
	...
	OUT		WILLOW_MSD_ATU_ENTRY **atuEntry
);
...
(
	...
	IN		MSD_U32 pkt_size,
	OUT	    MSD_U32 *numOfEntry,
	OUT		WILLOW_MSD_TCAM_DATA **tcamEntry
);
...
)
{
	...
	MSD_DBG_INFO(("Willow_msdRmuGetID Called.\n"));
	...
	*id = ((*(rspEthPacketPtr + 24 - delta) & 0xff) << 8) | (*(rspEthPacketPtr + 25 - delta) & 0xff);
	...
	MSD_DBG_INFO(("Willow_msdRmuGetID Exit.\n"));
	return MSD_OK;
}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuAtuDump
(
	IN MSD_QD_DEV *dev,
...
)
{
	...
	MSD_DBG_INFO(("Willow_msdRmuAtuDump Called.\n"));
	...
	*startAddr = next_cont_code;
	...
	MSD_DBG_INFO(("Willow_msdRmuAtuDump Exit.\n"));
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuMib2Dump
(
	IN MSD_QD_DEV *dev,
...
)
{
	...
	MSD_DBG_INFO(("Willow_msdRmuMib2Dump Called.\n"));
	...
	MSD_DBG_INFO(("Willow_msdRmuMib2Dump Exit.\n"));

	return retVal;
}

/*******************************************************************************
* Willow_msdWaitForBit
*
* DESCRIPTION:
*       Make the RMU unit wait for a bit of a register.
*
* INPUTS:
*       devAddr - device address
*		regAddr - register address
*       bitPosition - bit to wait for (0..15)
*       bitValue - bit value to wait for (0..1)
*
* OUTPUTS:
*       none.
*
...
*
*******************************************************************************/
MSD_STATUS Willow_msdWaitForBit
(
    IN  MSD_U8	 devNum,
    IN  MSD_U8    devAddr,
    IN  MSD_U8    regAddr,
    IN  MSD_U8    bitPosition,
    IN  MSD_U8    bitValue
)
{
	MSD_STATUS retVal = MSD_OK; /* make the compiler happy */
	MSD_U16 data = 2; /* clearly not 0 or 1 */
	MSD_U32	index;
	MSD_QD_DEV* dev = sohoDevGet(devNum);

	MSD_DBG_INFO(("Willow_msdWaitForBit Called.\n"));

	if (bitPosition > 15)
	{
		MSD_DBG_ERROR(("bitPosition out of range.\n"));
		return MSD_BAD_PARAM;
	}
	if (bitValue > 1)
	{
		MSD_DBG_ERROR(("bitValue out of range.\n"));
		return MSD_BAD_PARAM;
	}
	...
	if (!IS_RMU_SUPPORTED(dev)) {
		while (data != bitValue)
		{
			retVal = msdGetAnyRegField(devNum, devAddr, regAddr, bitPosition, 1, &data);
			if (retVal != MSD_OK)
			{
				return retVal;
			}
		...
		if (dev->accessMode == MSD_RMU_ACCESS_MODE_SYNCHRONOUS) {
			retVal = msdRmuRegWait(dev, devAddr, regAddr, bitPosition, bitValue);
		}
		else {
			index = dev->nCmdBufferLevel;
			dev->regCmdBuffer[index].devAddr = devAddr;
			dev->regCmdBuffer[index].regAddr = regAddr;
			dev->regCmdBuffer[index].data = bitPosition;
			dev->regCmdBuffer[index].isWaitOnBit = MSD_RMU_WAIT_ON_BIT_TRUE;
			if (bitValue == 0) {
				dev->regCmdBuffer[index].opCode = MSD_RMU_WAIT_ON_BIT_VAL0;
			}
			else {
				dev->regCmdBuffer[index].opCode = MSD_RMU_WAIT_ON_BIT_VAL1;
			}
			/* no return value for wait */
	...
	MSD_DBG_INFO(("Willow_msdWaitForBit Exit.\n"));
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuReadRegister
(
	IN MSD_QD_DEV *dev,
	IN MSD_U8 devAddr,
	IN MSD_U8 regAddr,
...
)
{
	...
	MSD_DBG_INFO(("Willow_msdRmuReadRegister Called.\n"));
	...
	MSD_DBG_INFO(("Willow_msdRmuReadRegister Exit.\n"));
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuWriteRegister
(
	IN MSD_QD_DEV *dev,
	IN MSD_U8 devAddr,
	IN MSD_U8 regAddr,
...
)
{
	...
	MSD_DBG_INFO(("Willow_msdRmuWriteRegister Called.\n"));
	...
	MSD_DBG_INFO(("Willow_msdRmuWriteRegister Exit.\n"));
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuMultiRegAccess
(
	IN MSD_QD_DEV *dev,
	INOUT MSD_RegCmd **regCmdList,
	IN MSD_U32 nCmd
)
{
	...
	MSD_DBG_INFO(("Willow_msdRmuMultiRegAccess Called.\n"));
	...
	MSD_DBG_INFO(("Willow_msdRmuMultiRegAccess Exit.\n"));
	return retVal;
}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuTCAMDump
(
	IN MSD_QD_DEV *dev,
	INOUT MSD_U32 *startAddr,
	OUT MSD_U32 *numOfEntry,
...
)
{
	...
	MSD_DBG_INFO(("Willow_msdRmuTCAMDump Called.\n"));
	...
	*startAddr = dump_TCAMPacket(delta, rspEthPacketPtr, rsp_pktlen, numOfEntry, tcamEntry);
	...
	MSD_DBG_INFO(("Willow_msdRmuTCAMDump Exit.\n"));
	return MSD_OK;
}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuTCAMWrite
(
	IN MSD_QD_DEV *dev,
	IN MSD_U32 tcamPointer,
	IN WILLOW_MSD_TCAM_DATA *tcamEntry
)
{
	MSD_Packet ReqPkt;
	MSD_U32 nCmd = 1;
	...
	MSD_DBG_INFO(("Willow_msdRmuTCAMWrite Called.\n"));

	if (tcamPointer >= WILLOW_MAX_IGR_TCAM_ENTRY || tcamEntry == NULL)
	{
	...
	MSD_DBG_INFO(("Willow_msdRmuTCAMWrite Exit.\n"));
	return MSD_OK;
}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuTCAMRead
(
	IN MSD_QD_DEV *dev,
	IN MSD_U32 tcamPointer,
	OUT WILLOW_MSD_TCAM_DATA *tcamEntry
)
{
	...
	MSD_DBG_INFO(("Willow_rmuTCAMRead Called.\n"));
	...
	MSD_DBG_INFO(("Willow_msdRmuTCAMRead Exit.\n"));
	return MSD_OK;
}

/*******************************************************************************
* Willow_msdRmuRegDump
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuRegDump
(
    IN MSD_QD_DEV *dev,
    IN  MSD_U8 devAddr,
...
)
{
    ...
    MSD_DBG_INFO(("Willow_msdRmuRegDump Called.\n"));
    ...
    if ((retVal = Willow_msdRmuMultiRegAccess(dev, &pRegCmd, nCmd)) != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_msdRmuMultiRegAccess returned fail.\n"));
        return retVal;
    }
    ...
    MSD_DBG_INFO(("Willow_msdRmuRegDump Exit.\n"));
    return MSD_OK;
}
