...
)
{
	...
	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Willow_gtcamLoadEntryIntf is NULL. \n"));
	...
	return Willow_gtcamLoadEntry(dev, tcamPointer, &tcam);
}
/*******************************************************************************
...
)
{
	...
	retVal=Willow_gtcamReadTCAMData(dev, tcamPointer, &tcam);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Willow_gtcamReadTCAMDataIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
...
)
{
	...
	retVal = Willow_gtcamGetNextTCAMData(dev, tcamPointer, &tcam);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Willow_gtcamGetNextTCAMDataIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	msdMemSet((void*)(tcamData), 0, sizeof(MSD_TCAM_DATA));
...
)
{
    ...
    msdMemSet((void*)(&tcam), 0, sizeof(WILLOW_MSD_TCAM_DATA));

	retVal = Willow_gtcamFindEntry(dev, tcamPointer, &tcam, found);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Willow_gtcamFindEntryIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	msdMemSet((void*)(tcamData), 0, sizeof(MSD_TCAM_DATA));
...
{
    return Willow_gtcamSetMode(dev, portNum, (WILLOW_MSD_TCAM_MODE)mode);
}
/*******************************************************************************
* Willow_gtcamGetModeIntf
*
* DESCRIPTION:
*       Set the TCAM Mode, The following mode is supported:
*               TCAM_MODE_DISABLE, TCAM_MODE_ENABLE_48, TCAM_MODE_ENABLE_96
...
{
    MSD_STATUS retVal;
    WILLOW_MSD_TCAM_MODE temp;

    if (mode == NULL)
    {
        MSD_DBG_ERROR(("Input param MSD_TCAM_MODE in Willow_gtcamGetModeIntf is NULL. \n"));
        return MSD_BAD_PARAM;
    }

    retVal = Willow_gtcamGetMode(dev, portNum, &temp);
...
{
	return Willow_gtcamEgrFlushEntry(dev, port, tcamPointer);
}
/*******************************************************************************
...
{
	return Willow_gtcamEgrFlushEntryAllPorts(dev, tcamPointer);
}
/*******************************************************************************
...
{
	WILLOW_MSD_TCAM_EGR_DATA  tcam;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_EGR_DATA in Willow_gtcamEgrLoadEntryIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	...
	return Willow_gtcamEgrLoadEntry(dev, port, tcamPointer, &tcam);
}
/*******************************************************************************
...
)
{
	...
	retVal = Willow_gtcamEgrGetNextTCAMData(dev, port, tcamPointer, &tcam);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_EGR_DATA in Willow_gtcamEgrGetNextTCAMDataIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
...
)
{
	...
	retVal = Willow_gtcamEgrReadTCAMData(dev, port, tcamPointer, &tcam);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_EGR_DATA in Willow_gtcamEgrReadTCAMDataIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	msdMemSet((void*)(tcamData), 0, sizeof(MSD_TCAM_EGR_DATA));
...
{
    WILLOW_MSD_TCAM_ENT_KEY tmpKeyMask;
    WILLOW_MSD_TCAM_ENT_KEY tmpKey;
    WILLOW_MSD_TCAM_ENT_ACT tmpAction;
    ...
    return Willow_gtcamAdvConfig(dev, pktType, tcamPointer, tcamPointer2, &tmpKeyMask, &tmpKey, maskPtr, patternPtr, &tmpAction, entry2Used);
}
#endif
