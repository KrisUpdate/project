...
*
*******************************************************************************/
MSD_STATUS Willow_msdSetSMIC45PhyReg
(
    IN MSD_QD_DEV *dev,
    IN MSD_U8    devAddr,
    IN MSD_U8    phyAddr,
...
)
{
    ...
    MSD_DBG(("(LOG WILLOW SMI PHY C45 RW): devAddr 0x%02x, phyAddr 0x%02x, regAddr 0x%02x, ",
    ...
    if (!IS_RMU_SUPPORTED(dev))
    {
        ...
        do
        {
            if (msdGetAnyReg(dev->devNum, WILLOW_GLOBAL2_DEV_ADDR, WILLOW_QD_REG_SMI_PHY_CMD, &smiReg) != MSD_OK)
            {
    ...
    if(msdSetAnyReg(dev->devNum, WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_SMI_PHY_DATA, regAddr) != MSD_OK)
    {
        msdRegisterAccessFlush(dev->devNum);
    ...
    if(msdSetAnyReg(dev->devNum, WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_SMI_PHY_CMD, smiReg) != MSD_OK)
    {
        msdRegisterAccessFlush(dev->devNum);
        MSD_DBG_ERROR(("Write WILLOW_QD_REG_SMI_PHY_CMD register returned: %s.\n", msdDisplayStatus(MSD_FAIL)));
        return MSD_FAIL;
    }
    ...
    if (!IS_RMU_SUPPORTED(dev))
    {
        do 
        {
            ...
            if(timeOut-- < 1 ) 
            {
                MSD_DBG_INFO(("read SMI Phy WILLOW_QD_REG_SMI_PHY_CMD register Timed Out\n"));
                return MSD_FAIL;
            }
        } while (smiReg & MSD_SMI_BUSY);
    }
    ...
    if(msdSetAnyReg(dev->devNum, WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_SMI_PHY_DATA, data) != MSD_OK)
    {
        msdRegisterAccessFlush(dev->devNum);
        MSD_DBG_ERROR(("write WILLOW_QD_REG_SMI_PHY_DATA register returned: %s.\n", msdDisplayStatus(MSD_FAIL)));
        return MSD_FAIL;
...
*
*******************************************************************************/
MSD_STATUS Willow_msdGetSMIC45PhyReg
(
    IN  MSD_QD_DEV *dev,
    IN  MSD_U8     devAddr,
    IN  MSD_U8     phyAddr,
...
)
{
    ...
    if (!IS_RMU_SUPPORTED(dev))
    {
        ...
        do
        {
            ...
            if (timeOut-- < 1)
            {
                MSD_DBG_INFO(("read WILLOW_QD_REG_SMI_PHY_CMD register Timed Out\n"));
                return MSD_FAIL;
            }
        } while (smiReg & MSD_SMI_BUSY);
    }
    ...
    {
        msdRegisterAccessFlush(dev->devNum);
        MSD_DBG_ERROR(("Write WILLOW_QD_REG_SMI_PHY_CMD register Failed\n"));
        return MSD_FAIL;
    }
    ...
    if (!IS_RMU_SUPPORTED(dev))
    {
        ...
        do
        {
            ...
            if (timeOut-- < 1)
            {
                MSD_DBG_ERROR(("read WILLOW_QD_REG_SMI_PHY_CMD register Timed Out\n"));
                return MSD_FAIL;
            }
        } while (smiReg & MSD_SMI_BUSY);
    }
    ...
    if(msdGetAnyReg(dev->devNum, WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_SMI_PHY_DATA, &tmp) != MSD_OK)
    {
        msdRegisterAccessFlush(dev->devNum);
        MSD_DBG_ERROR(("read WILLOW_QD_REG_SMI_PHY_DATA register returned: %s.\n", msdDisplayStatus(MSD_FAIL)));
        return MSD_FAIL;
    }
    if (msdRegisterAccessFlush(dev->devNum) != MSD_OK)
    {
        MSD_DBG_ERROR(("read WILLOW_QD_REG_SMI_PHY_DATA register returned: %s.\n", msdDisplayStatus(MSD_FAIL)));
        return MSD_FAIL;
    }
    *data = tmp;
    ...
    MSD_DBG(("(LOG WILLOW SMI PHY C45 RR): devAddr 0x%02x, phyAddr 0x%02x, regAddr 0x%02x, ",
        devAddr, phyAddr, regAddr));
    MSD_DBG(("data 0x%04x.\n", *data));
...
*
*******************************************************************************/
MSD_STATUS Willow_msdSetSMIPhyReg
(
    IN MSD_QD_DEV *dev,
    IN  MSD_U8    devAddr,
    IN  MSD_U8    regAddr,
...
)
{
    ...
    MSD_DBG(("(LOG WILLOW SMI PHY C22 RW): devAddr 0x%02x, regAddr 0x%02x, ",
        devAddr, regAddr));
    MSD_DBG(("data 0x%04x.\n", data));
...
*
*******************************************************************************/
MSD_STATUS Willow_msdGetSMIPhyReg
(
    IN MSD_QD_DEV *dev,
    IN  MSD_U8    devAddr,
    IN  MSD_U8    regAddr,
...
)
{
    ...
    if (!IS_RMU_SUPPORTED(dev))
    {
        ...
        do 
        {
            ...
            if(timeOut-- < 1 ) 
            {
                MSD_DBG_ERROR(("Read WILLOW_QD_REG_SMI_PHY_CMD Timed Out\n"));
                return MSD_FAIL;
            }
        } while (smiReg & MSD_SMI_BUSY);
    }
    ...
    if(msdGetAnyReg(dev->devNum, WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_SMI_PHY_DATA, &smiReg) != MSD_OK)
    {
        msdRegisterAccessFlush(dev->devNum);
...
*
*******************************************************************************/
MSD_STATUS Willow_msdSetSMIPhyRegField
(
    IN MSD_QD_DEV *dev,
    IN  MSD_U8    devAddr,
    IN  MSD_U8    regAddr,
...
)
{
    ...
    retVal =  Willow_msdGetSMIPhyReg(dev,devAddr,regAddr,&tmpData);
    if(retVal != MSD_OK)
    {
        return retVal;
    }
    ...
    retVal = Willow_msdSetSMIPhyReg(dev,devAddr,regAddr,tmpData);
...
*
*******************************************************************************/
MSD_STATUS Willow_msdGetSMIPhyRegField
(
    IN  MSD_QD_DEV *dev,
    IN  MSD_U8    devAddr,
    IN  MSD_U8    regAddr,
...
MSD_STATUS Willow_msdSetPagedPhyRegField
(
    IN  MSD_QD_DEV *dev,
    IN  MSD_U8    portNum,
    IN  MSD_U8    pageNum,
...
{
	MSD_STATUS retVal;
	retVal = Willow_msdSetSMIPhyReg(dev, portNum, 22, pageNum);
	if(retVal != MSD_OK)
		return retVal;
	retVal = Willow_msdSetSMIPhyRegField(dev, portNum, regAddr, fieldOffset, fieldLength, data);
	return retVal;
}	

MSD_STATUS Willow_msdGetPagedPhyRegField
(
    IN  MSD_QD_DEV *dev,
    IN  MSD_U8    portNum,
    IN  MSD_U8    pageNum,
...
)	
{
	...
	if(retVal != MSD_OK)
		return retVal;
	retVal = Willow_msdGetSMIPhyRegField(dev, portNum, regAddr, fieldOffset, fieldLength, data);
	return retVal;
}

MSD_STATUS Willow_msdWritePagedPhyReg
(
    IN  MSD_QD_DEV *dev,
    IN  MSD_U8    portNum,
    IN  MSD_U8    pageNum,
...
)
{
	...
	if(retVal != MSD_OK)
		return retVal;
	retVal = Willow_msdSetSMIPhyReg(dev, portNum, regAddr, data);
	return retVal;
}

MSD_STATUS Willow_msdReadPagedPhyReg
(
    IN  MSD_QD_DEV *dev,
    IN  MSD_U8    portNum,
    IN  MSD_U8    pageNum,
...
)
{
	...
	if(retVal != MSD_OK)
		return retVal;
	retVal = Willow_msdGetSMIPhyReg(dev, portNum, regAddr, data);
	return retVal;
}

MSD_STATUS Willow_msdSetSMIPhyXMDIOReg
(
    IN MSD_QD_DEV *dev,
    IN MSD_U8  portNum,
    IN MSD_U8  devAddr,
...
)
{
    ...
    retVal = Willow_msdWritePagedPhyReg(dev, hwPort, 0, 13, tmpData);
    if (retVal != MSD_OK)
    {
    ...
    tmpData = regAddr;
    retVal = Willow_msdWritePagedPhyReg(dev, hwPort, 0, 14, tmpData);
    if (retVal != MSD_OK)
    {
    ...
    retVal = Willow_msdWritePagedPhyReg(dev, hwPort, 0, 14, data);
    if (retVal != MSD_OK)
    {
...
MSD_STATUS Willow_msdGetSMIPhyXMDIOReg
(
    IN MSD_QD_DEV *dev,
    IN MSD_U8  portNum,
    IN MSD_U8  devAddr,
...
)
{
    ...
    /* Set MMD access control address is Address | devAddr */
    tmpData = MSD_PHY_MMD_ADDR | devAddr;
    retVal = Willow_msdWritePagedPhyReg(dev, portNum, 0, 13, tmpData);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("ERROR to Set MMD access control address is Address | devAddr.\n"));
        return retVal;
    ...
    /* Set MMD access Data is regAddr */
    tmpData = regAddr;
    retVal = Willow_msdWritePagedPhyReg(dev, portNum, 0, 14, tmpData);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("ERROR to Set MMD access Data is regAddr.\n"));
        return retVal;
    ...
    retVal = Willow_msdReadPagedPhyReg(dev, portNum, 0, 14, &tmpData);
    if (retVal != MSD_OK)
    {
...
)
{
    ...
    /*if((retVal=msdGetSMIPhyReg(dev,portNum,0,&tmpData)) 
           != MSD_OK)*/
	if((retVal=Willow_msdReadPagedPhyReg(dev, portNum, pageNum, 0,&tmpData)) 
           != MSD_OK)
    {
    ...
    /*if((retVal=msdSetSMIPhyReg(dev,portNum,0,tmpData)) 
        != MSD_OK)*/
	if((retVal=Willow_msdWritePagedPhyReg(dev, portNum, pageNum, 0,tmpData)) 
        != MSD_OK)
    {
    ...
    for (retryCount = 0x1000; retryCount > 0; retryCount--)
    {
		...
		if((retVal=Willow_msdReadPagedPhyReg(dev, portNum, pageNum, 0,&tmpData)) != MSD_OK)
        {
            MSD_DBG_INFO(("Reading register failed\n"));
            return retVal;
        }
