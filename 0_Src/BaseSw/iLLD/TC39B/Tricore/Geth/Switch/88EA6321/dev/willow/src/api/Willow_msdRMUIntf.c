...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuGetIDIntf
(
IN  MSD_QD_DEV *dev,
OUT MSD_U16 *id
)
{
	return Willow_msdRmuGetID(dev, id);
}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuAtuDumpIntf
(
	IN MSD_QD_DEV *dev,
...
{
	WILLOW_MSD_ATU_ENTRY entry[MSD_RMU_MAX_ATUS];
	WILLOW_MSD_ATU_ENTRY *entryPtr = &(entry[0]);
	MSD_STATUS retVal;
	int i;

	for (i = 0; i < MSD_RMU_MAX_ATUS; i++)
	{
		msdMemSet((void*)(entry + i), 0, sizeof(WILLOW_MSD_ATU_ENTRY));
	}

	retVal = Willow_msdRmuAtuDump(dev, startAddr, numOfEntry, &entryPtr);
	if (MSD_OK != retVal)
		return retVal;

	if ((*numOfEntry > 0) && (NULL == atuEntry))
	{
		MSD_DBG_ERROR(("Input param MSD_ATU_ENTRY** in Willow_rmuAtuDumpIntf is NULL. \n"));
	...
	for (i = 0; i < (int)(*numOfEntry); i++)
	{
		if (NULL == ((*atuEntry) + i))
		{
			MSD_DBG_ERROR(("Input param MSD_ATU_ENTRY* for index %d in Willow_rmuAtuDumpIntf is NULL. \n", i));
			return MSD_BAD_PARAM;
		}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuMib2DumpIntf
(
	IN MSD_QD_DEV *dev,
...
)
{
	...
	retVal = Willow_msdRmuMib2Dump(dev, enClearOnRead, port, &statsCounter);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == statsCounterSet)
	{
		MSD_DBG_ERROR(("Input param MSD_STATS_COUNTER_SET in Willow_msdRmuMib2DumpIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuReadRegisterIntf
(
IN MSD_QD_DEV *dev,
IN MSD_U8 devAddr,
IN MSD_U8 regAddr,
...
{
	return Willow_msdRmuReadRegister(dev,devAddr,regAddr,data);
}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuWriteRegisterIntf
(
IN MSD_QD_DEV *dev,
IN MSD_U8 devAddr,
IN MSD_U8 regAddr,
...
{
	return Willow_msdRmuWriteRegister(dev, devAddr, regAddr, data);
}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuMultiRegAccessIntf
(
IN MSD_QD_DEV *dev,
...
{
	return Willow_msdRmuMultiRegAccess(dev,regCmdList,nCmd);
}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuTCAMDumpIntf
(
	IN MSD_QD_DEV *dev,
	INOUT MSD_U32 *startAddr,
	OUT MSD_U32 *numOfEntry,
...
{
	MSD_STATUS           retVal;
	WILLOW_MSD_TCAM_DATA tcam[MSD_RMU_MAX_TCAMS];
	WILLOW_MSD_TCAM_DATA *tcamPtr = &(tcam[0]);

	MSD_U8 i = 0, j = 0;

	for (i = 0; i < MSD_RMU_MAX_TCAMS; i++)
		msdMemSet((void*)(tcam + i), 0, sizeof(WILLOW_MSD_TCAM_DATA));

	retVal = Willow_msdRmuTCAMDump(dev, startAddr, numOfEntry, &tcamPtr);
	if (MSD_OK != retVal)
		return retVal;

	if ((*numOfEntry > 0) && (NULL == tcamEntry))
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA** in Willow_msdRmuTCAMDumpIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	...
	for (i = 0; i < (int)(*numOfEntry); i++)
	{
		if (NULL == ((*tcamEntry) + i))
		{
			MSD_DBG_ERROR(("Input param MSD_TCAM_DATA* for index %d in Willow_msdRmuTCAMDumpIntf is NULL. \n", i));
			return MSD_BAD_PARAM;
		}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuTCAMWriteIntf
(
	IN MSD_QD_DEV *dev,
...
{
	WILLOW_MSD_TCAM_DATA tcam;
	MSD_U8 i = 0;

	if (NULL == tcamData)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Willow_msdRmuTCAMWriteIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	...
	return Willow_msdRmuTCAMWrite(dev, tcamPointer, &tcam);
}
...
*
*******************************************************************************/
MSD_STATUS Willow_msdRmuTCAMReadIntf
(
IN MSD_QD_DEV *dev,
IN MSD_U32 tcamPointer,
OUT MSD_TCAM_DATA *tcamEntry
)
{
	...
	msdMemSet(&tcam, 0, sizeof(WILLOW_MSD_TCAM_DATA));

	retVal = Willow_msdRmuTCAMRead(dev, tcamPointer, &tcam);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == tcamEntry)
	{
		MSD_DBG_ERROR(("Input param MSD_TCAM_DATA in Willow_msdRmuTCAMReadIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
