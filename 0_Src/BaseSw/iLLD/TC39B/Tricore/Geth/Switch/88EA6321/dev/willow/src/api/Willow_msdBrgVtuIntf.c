...
)
{
	MSD_U8               port;
	MSD_LPORT               lport;
	WILLOW_MSD_VTU_ENTRY tempEntry;
	MSD_STATUS           retVal;

    if (NULL == entry || NULL == found)
	{
		MSD_DBG_ERROR(("Input pointer param MSD_VTU_ENTRY or found in Willow_gvtuFindVidEntryIntf is NULL. \n"));
    ...
    msdMemSet((void*)(&tempEntry), 0, sizeof(WILLOW_MSD_VTU_ENTRY));
    tempEntry.vid = vlanId & 0xfff;
    tempEntry.vidExInfo.vtuPage = (vlanId >> 12) & 0x1;
	

    retVal = Willow_gvtuFindVidEntry(dev, &tempEntry, found);
...
)
{
    ...
    if (NULL == isExists)
    {
        MSD_DBG_ERROR(("Input pointer param isExists in Willow_gvtuExistVidEntryIntf is NULL. \n"));
        return MSD_BAD_PARAM;
    }
    if (vlanId > 0x1fff)
    {
    ...
    retVal = Willow_gvtuFindVidEntry(dev, &tempEntry, isExists);
...
)
{
	...
	if (NULL == vtuEntry)
	{
		MSD_DBG_ERROR(("Input param MSD_VTU_ENTRY in Willow_gvtuAddEntryIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	...
	return Willow_gvtuAddEntry(dev, &entry);
}
/*******************************************************************************
* Willow_gvtuDelEntry
*
...
)
{
    ...
    msdMemSet((void*)(&entry), 0, sizeof(WILLOW_MSD_VTU_ENTRY));
    entry.vid = vlanId & 0xfff;
    entry.vidExInfo.vtuPage = (vlanId >> 12) & 0x1;

	return Willow_gvtuDelEntry(dev, &entry);
}
/*******************************************************************************
* Willow_gvtuSetMode
*
...
{
	return Willow_gvtuSetMode(dev, (WILLOW_MSD_VTU_MODE)mode);
}
/*******************************************************************************
* Willow_gvtuGetMode
*
...
{
	return Willow_gvtuGetMode(dev, (WILLOW_MSD_VTU_MODE*)mode);
}
/*******************************************************************************
...
{
	WILLOW_MSD_VTU_INT_STATUS vtuint;
	MSD_STATUS           retVal;

    if (NULL == vtuIntStatus)
    {
        MSD_DBG_ERROR(("Input param MSD_VTU_INT_STATUS in Willow_gvtuGetViolationIntf is NULL. \n"));
        return MSD_BAD_PARAM;
    }

	msdMemSet((void*)(&vtuint), 0, sizeof(WILLOW_MSD_VTU_INT_STATUS));
    msdMemSet((void*)vtuIntStatus, 0, sizeof(MSD_VTU_INT_STATUS));

	retVal = Willow_gvtuGetViolation(dev, &vtuint);
...
)
{
    ...
    if (NULL == count)
    {
        MSD_DBG_ERROR(("Input pointer param count in Willow_gvtuGetEntryCountIntf is NULL. \n"));
        return MSD_BAD_PARAM;
    }
    ...
    *count = 0;
    ...
    while (retVal == MSD_OK)
    {
        retVal = Willow_gvtuGetEntryNext(dev, &tempEntry);
...
)
{
    ...
    {
        MSD_DBG_ERROR(("Failed (Bad Port or Bad vlanId) in Willow_gvtuSetMemberTagIntf.\n"));
        return MSD_BAD_PARAM;
    }
    ...
    retVal = Willow_gvtuAddEntry(dev, &tempEntry);
...
)
{
	...
	if (NULL == vtuEntry)
	{
		MSD_DBG_ERROR(("Input param MSD_VTU_ENTRY in Willow_gfdbGetAtuEntryNextIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	...
	retVal = Willow_gvtuGetEntryNext(dev, &entry);
	if (MSD_OK != retVal)
		return retVal;
