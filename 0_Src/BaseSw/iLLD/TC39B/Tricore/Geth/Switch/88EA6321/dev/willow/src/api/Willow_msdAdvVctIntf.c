...
{
	WILLOW_MSD_ADV_VCT_MODE vctmode;
	WILLOW_MSD_ADV_CABLE_STATUS cable;
	MSD_STATUS  retVal;
	MSD_U8 i = 0, j = 0;
	...
	retVal=Willow_gvctGetAdvCableDiag(dev, port, vctmode, &cable);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == cableStatus)
	{
		MSD_DBG_ERROR(("Input param MSD_ADV_CABLE_STATUS in Willow_gvctGetAdvCableDiagIntf is NULL. \n"));
...
{
	WILLOW_MSD_ADV_EXTENDED_STATUS extend;
	MSD_STATUS retVal;
	MSD_U8 i = 0;

	retVal = Willow_gvctGetAdvExtendedStatus(dev, port, &extend);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == extendedStatus)
	{
		MSD_DBG_ERROR(("Input param MSD_ADV_EXTENDED_STATUS in Willow_gvctGetAdvExtendedStatusIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
