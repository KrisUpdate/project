...
/*
 *  typedef: struct WILLOW_MSD_TCAM_PG_DATA
 *
...
} WILLOW_MSD_TCAM_PG_DATA;

/*
 *  typedef: struct WILLOW_MSD_TCAM_DATA_HW
 *
...
{
	/* Pg0 registers */
	WILLOW_MSD_TCAM_PG_DATA pg0;
	/* Pg1 registers */
	WILLOW_MSD_TCAM_PG_DATA pg1;
	/* Pg2 registers */
	WILLOW_MSD_TCAM_PG_DATA pg2;
} WILLOW_MSD_TCAM_DATA_HW;

/*
 *  typedef: struct WILLOW_MSD_TCAM_EGR_DATA_HW
 *
...
} WILLOW_MSD_TCAM_EGR_DATA_HW;

typedef struct
{
    MSD_U32    tcamEntry;
    WILLOW_MSD_TCAM_DATA_HW    tcamDataP; 
} WILLOW_MSD_TCAM_OP_DATA;

typedef struct 
{
	MSD_U8  port;
	MSD_U32	  tcamEntry;
	WILLOW_MSD_TCAM_EGR_DATA_HW	tcamDataP;
} WILLOW_MSD_TCAM_EGR_OP_DATA;

typedef enum
{
	Willow_TCAM_FLUSH_ALL = 0x1,
	Willow_TCAM_FLUSH_ENTRY = 0x2,
	Willow_TCAM_LOAD_ENTRY = 0x3,
	Willow_TCAM_PURGE_ENTRY = 0x3,
	Willow_TCAM_GET_NEXT_ENTRY = 0x4,
	Willow_TCAM_READ_ENTRY = 0x5
} WILLOW_MSD_TCAM_OPERATION;

/****************************************************************************/
/* TCAM operation function declaration.                                    */
/****************************************************************************/
static MSD_STATUS Willow_tcamOperationPerform
(
    IN   MSD_QD_DEV             *dev,
    IN   WILLOW_MSD_TCAM_OPERATION    tcamOp,
    INOUT WILLOW_MSD_TCAM_OP_DATA    *opData
);

static MSD_STATUS Willow_tcamEgrOperationPerform
(
    IN   MSD_QD_DEV             *dev,
    IN   WILLOW_MSD_TCAM_OPERATION    tcamOp,
    INOUT WILLOW_MSD_TCAM_EGR_OP_DATA    *opData
);
static MSD_STATUS Willow_setTcamHWData
(
    IN	WILLOW_MSD_TCAM_DATA	*iData,
	OUT	WILLOW_MSD_TCAM_DATA_HW	*oData
);
static MSD_STATUS Willow_getTcamHWData
(
	IN	WILLOW_MSD_TCAM_DATA_HW	*iData,
	OUT	WILLOW_MSD_TCAM_DATA	*oData
);
static MSD_STATUS Willow_setTcamEgrHWData
(
	IN	WILLOW_MSD_TCAM_EGR_DATA	*iData,
	OUT	WILLOW_MSD_TCAM_EGR_DATA_HW	*oData
);
static MSD_STATUS Willow_getTcamEgrHWData
(
	IN	WILLOW_MSD_TCAM_EGR_DATA_HW	*iData,
	OUT	WILLOW_MSD_TCAM_EGR_DATA	*oData
);
static void displayTcamData(WILLOW_MSD_TCAM_DATA *tcamData);
static void mappingIpv4ToTcam(WILLOW_MSD_TCAM_DATA *tcamData, MSD_TCAM_ENT_IPV4_UNT *pIpv4Data, MSD_TCAM_ENT_IPV4_UNT *pIpv4Mask);
static void mappingUdpToTcam(int startIndex, WILLOW_MSD_TCAM_DATA *tcamData, MSD_TCAM_ENT_UDP_UNT *pUdpData, MSD_TCAM_ENT_UDP_UNT *pUdpMask);
static void mappingActionToTcam(WILLOW_MSD_TCAM_DATA *tcamData, WILLOW_MSD_TCAM_ENT_ACT *actionPtr);
static void mappingIpv6ToTcam(WILLOW_MSD_TCAM_DATA *tcamData, WILLOW_MSD_TCAM_DATA *tcamData2, MSD_TCAM_ENT_IPV6_UNT *pIpv6, MSD_TCAM_ENT_IPV6_UNT *pIpv6Mask);
static void mappingTcpOverIpv4ToTcam(WILLOW_MSD_TCAM_DATA *tcamData, WILLOW_MSD_TCAM_DATA *tcamData2, MSD_TCAM_ENT_TCP_UNT *pTcpData, MSD_TCAM_ENT_TCP_UNT *pTcpMask);
static void mappingTcpOverIpv6ToTcam(WILLOW_MSD_TCAM_DATA *tcamData2, MSD_TCAM_ENT_TCP_UNT *pTcpData, MSD_TCAM_ENT_TCP_UNT *pTcpMask);
static MSD_BOOL checkEntry2Used(WILLOW_MSD_TCAM_DATA *tcamData2);
...
{
    MSD_STATUS           retVal;
    WILLOW_MSD_TCAM_OPERATION    op;
    WILLOW_MSD_TCAM_OP_DATA     tcamOpData;

    MSD_DBG_INFO(("Willow_gtcamFlushAll Called.\n"));

    /* Program Tuning register */
    op = Willow_TCAM_FLUSH_ALL;
    retVal = Willow_tcamOperationPerform(dev,op, &tcamOpData);
    if(retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Willow_gtcamFlushAll Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamFlushEntry Called.\n"));

    /* check if the given pointer is valid */
    if (tcamPointer >= WILLOW_MAX_IGR_TCAM_ENTRY)
	{
		MSD_DBG_ERROR(("Bad tcamPointer %u. It should be within [0, %d].\n", (unsigned int)tcamPointer, (int)(WILLOW_MAX_IGR_TCAM_ENTRY - 1)));
        return MSD_BAD_PARAM;
	}

    /* Program Tuning register */
    op = Willow_TCAM_FLUSH_ENTRY;
    ...
    MSD_DBG_INFO(("Willow_gtcamFlushEntry Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamLoadEntry Called.\n"));

    /* check if the given pointer is valid */
	if ((tcamPointer >= WILLOW_MAX_IGR_TCAM_ENTRY) || (tcamData == NULL))
	{
    ...
    /* Program Tuning register */
    op = Willow_TCAM_LOAD_ENTRY;
    tcamOpData.tcamEntry = tcamPointer;
    /*tcamOpData.tcamDataP = tcamData;*/
	Willow_setTcamHWData(tcamData, &tcamOpData.tcamDataP);
	retVal = Willow_tcamOperationPerform(dev, op, &tcamOpData);
    if(retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Willow_gtcamLoadEntry Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamReadTCAMData Called.\n"));
	...
	Willow_getTcamHWData(&tcamOpData.tcamDataP,tcamData);

    MSD_DBG_INFO(("Willow_gtcamReadTCAMData Exit.\n"));
    return MSD_OK;
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamGetNextTCAMData Called.\n"));

    /* check if the given pointer is valid */
	if (tcamPointer == NULL || *tcamPointer >= WILLOW_MAX_IGR_TCAM_ENTRY || tcamData == NULL)
	{
    ...
    if(retVal == MSD_NO_SUCH)
	{
		...
		MSD_DBG_INFO(("Willow_gtcamGetNextTCAMData Exit.\n"));
		return retVal;
	}	
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamFindEntry Called.\n"));

    /* check if the given pointer is valid */
	if (tcamPointer >= WILLOW_MAX_IGR_TCAM_ENTRY || tcamData == NULL || found == NULL)
	{
	...
	*found = MSD_FALSE;
    ...
    if(retVal == MSD_NO_SUCH )
	{
		MSD_DBG_INFO(("Not found this entry, Willow_gtcamFindEntry Exit.\n"));
		return MSD_OK;
	}	
	...
	*found = MSD_TRUE;
	Willow_getTcamHWData(&tcamOpData.tcamDataP,tcamData);
    MSD_DBG_INFO(("Willow_gtcamFindEntry Exit.\n"));
    return MSD_OK;
}
/*******************************************************************************
* Willow_gtcamSetMode
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamSetMode Called.\n"));
    ...
    if (mode < WILLOW_MSD_TCAM_MODE_DISABLE || mode > WILLOW_MSD_TCAM_MODE_ENABLE_96)
    {
    ...
    MSD_DBG_INFO(("Willow_gtcamSetMode Exit.\n"));
    return MSD_OK;
}
/*******************************************************************************
* Willow_gtcamGetMode
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamGetMode Called.\n"));
    ...
    *mode = data;
    ...
    MSD_DBG_INFO(("Willow_gtcamGetMode Exit.\n"));
    return MSD_OK;
}
/*******************************************************************************
...
)
{
    MSD_STATUS retVal;
    MSD_BOOL found;
    WILLOW_MSD_TCAM_DATA  tcamData;
    MSD_U8 hwPort;         /* the physical port number     */

    MSD_DBG_INFO(("Willow_gtcamAddEntryPort Called.\n"));
    ...
    retVal = Willow_gtcamFindEntry(dev, tcamPointer, &tcamData, &found);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_gtcamFindEntry in Willow_gtcamAddEntryPort returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    retVal = Willow_gtcamLoadEntry(dev, tcamPointer, &tcamData);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_gtcamLoadEntry in Willow_gtcamAddEntryPort returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gtcamAddEntryPort Exit.\n"));
    return retVal;
}
/*******************************************************************************
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamRemoveEntryPort Called.\n"));
    ...
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_gtcamFindEntry in Willow_gtcamRemoveEntryPort returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_gtcamLoadEntry in Willow_gtcamRemoveEntryPort returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gtcamRemoveEntryPort Exit.\n"));
    return retVal;
}
...
)
{
    MSD_STATUS           retVal;
    WILLOW_MSD_TCAM_OPERATION    op;
    WILLOW_MSD_TCAM_EGR_OP_DATA     tcamOpData;
	MSD_U8			hwPort;

    MSD_DBG_INFO(("Willow_gtcamEgrFlushEntry Called.\n"));
    ...
    tcamOpData.tcamEntry = tcamPointer;
    retVal = Willow_tcamEgrOperationPerform(dev, op, &tcamOpData);
    if(retVal != MSD_OK)
    {
	MSD_DBG_ERROR(("Willow_tcamEgrOperationPerform FLUSH_ENTRY returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gtcamEgrFlushEntry Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamEgrFlushEntryAllPorts Called.\n"));
    ...
    if(retVal != MSD_OK)
    {
	MSD_DBG_ERROR(("Willow_tcamEgrOperationPerform FLUSH_ENTRY for all returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gtcamEgrFlushEntryAllPorts Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamEgrLoadEntry Called.\n"));
    ...
    tcamOpData.tcamEntry = tcamPointer;
    /*tcamOpData.tcamDataP = tcamData;*/
	Willow_setTcamEgrHWData(tcamData,&tcamOpData.tcamDataP);
    retVal = Willow_tcamEgrOperationPerform(dev,op, &tcamOpData);
    if(retVal != MSD_OK)
    {
	MSD_DBG_ERROR(("Willow_tcamEgrOperationPerform LOAD_ENTRY returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gtcamEgrLoadEntry Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamEgrGetNextTCAMData Called.\n"));
    ...
    {
        MSD_DBG_INFO(("No higher valid entry founded return MSD_NO_SUCH"));
        MSD_DBG_INFO(("Willow_gtcamEgrGetNextTCAMData Exit.\n"));
        return retVal;
    }	
    else if(retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_tcamEgrOperationPerform GET_NEXT returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    *tcamPointer = tcamOpData.tcamEntry;
    Willow_getTcamEgrHWData(&tcamOpData.tcamDataP,tcamData);
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamEgrReadTCAMData Called.\n"));
    ...
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Willow_tcamEgrOperationPerform READ_ENTRY returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

	Willow_getTcamEgrHWData(&tcamOpData.tcamDataP,tcamData);

    MSD_DBG_INFO(("Willow_gtcamEgrReadTCAMData Exit.\n"));
    return MSD_OK;
}
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gtcamDump Called.\n"));
    tcamPointer = 0xff;

    while ((status = Willow_gtcamGetNextTCAMData(dev, &tcamPointer, &tcamData)) == MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Willow_gtcamDump Exit.\n"));
    return MSD_OK;
...
)
{
    MSD_STATUS retVal;
    WILLOW_MSD_TCAM_DATA  tcamData;
    WILLOW_MSD_TCAM_DATA  tcamData2;
    int i;

    MSD_DBG_INFO(("Willow_gtcamAdvConfig Called.\n"));

    msdMemSet(&tcamData, 0, sizeof(WILLOW_MSD_TCAM_DATA));
    msdMemSet(&tcamData2, 0, sizeof(WILLOW_MSD_TCAM_DATA));
    ...
    if (checkEntry2Used(&tcamData2) == MSD_TRUE)
    {
        ...
        if (retVal != MSD_OK)
        {
            MSD_DBG_ERROR(("Willow_gtcamLoadEntry for tcamData returned: %s.\n", msdDisplayStatus(retVal)));
            return retVal;
        }
        retVal = Willow_gtcamLoadEntry(dev, tcamPointer2, &tcamData2);
        if (retVal != MSD_OK)
        {
            MSD_DBG_ERROR(("Willow_gtcamLoadEntry for tcamData2 returned: %s.\n", msdDisplayStatus(retVal)));
            return retVal;
        }
        *entry2Used = MSD_TRUE;
    }
    else
    {
        ...
        if (retVal != MSD_OK)
        {
            MSD_DBG_ERROR(("Willow_gtcamLoadEntry for tcamData only returned: %s.\n", msdDisplayStatus(retVal)));
            return retVal;
        }
        *entry2Used = MSD_FALSE;
    }

    MSD_DBG_INFO(("Willow_gtcamAdvConfig Exit.\n"));
    return MSD_OK;
}
...
)
{
	...
	msdMemSet(oData,0,sizeof(WILLOW_MSD_TCAM_DATA_HW));
...
{
	msdMemSet(oData,0,sizeof(WILLOW_MSD_TCAM_EGR_DATA_HW));
...
static MSD_STATUS Willow_tcamSetPage0Data(MSD_QD_DEV *dev, WILLOW_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<0x1c; i++)
	{
		retVal = msdSetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, (MSD_U8)i, tcamDataP->pg0.frame[i - 2]);
		if (retVal != MSD_OK)
		{
...
static MSD_STATUS Willow_tcamSetPage1Data(MSD_QD_DEV *dev, WILLOW_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<0x1c; i++)
	{
		retVal = msdSetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, (MSD_U8)i, tcamDataP->pg1.frame[i - 2]);
		if (retVal != MSD_OK)
		{
            msdRegisterAccessFlush(dev->devNum);
            return retVal;
...
static MSD_STATUS Willow_tcamSetPage2Data(MSD_QD_DEV *dev, WILLOW_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<9; i++)
	{
		retVal = msdSetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, (MSD_U8)i, tcamDataP->pg2.frame[i - 2]);
		if(retVal != MSD_OK)
		{
            msdRegisterAccessFlush(dev->devNum);
			return retVal;
...
static MSD_STATUS Willow_tcamSetPage3Data(MSD_QD_DEV *dev, WILLOW_MSD_TCAM_EGR_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<5; i++)
	{
		retVal = msdSetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, (MSD_U8)i, tcamDataP->frame[i - 2]);
		if(retVal != MSD_OK)
		{
            msdRegisterAccessFlush(dev->devNum);
            return retVal;
...
static MSD_STATUS Willow_tcamGetPage0Data(MSD_QD_DEV *dev, WILLOW_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<0x1c; i++)
	{
		retVal = msdGetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, (MSD_U8)i, &tcamDataP->pg0.frame[i - 2]);
		if(retVal != MSD_OK)
		{
            msdRegisterAccessFlush(dev->devNum);
            return retVal;
...
static MSD_STATUS Willow_tcamGetPage1Data(MSD_QD_DEV *dev, WILLOW_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<0x1c; i++)
	{
		retVal = msdGetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, (MSD_U8)i, &tcamDataP->pg1.frame[i - 2]);
		if(retVal != MSD_OK)
		{
            msdRegisterAccessFlush(dev->devNum);
            return retVal;
...
static MSD_STATUS Willow_tcamGetPage2Data(MSD_QD_DEV *dev, WILLOW_MSD_TCAM_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<9; i++)
	{
		retVal = msdGetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, (MSD_U8)i, &tcamDataP->pg2.frame[i - 2]);
		if (retVal != MSD_OK)
		{
            msdRegisterAccessFlush(dev->devNum);
            return retVal;
...
static MSD_STATUS Willow_tcamGetPage3Data(MSD_QD_DEV *dev, WILLOW_MSD_TCAM_EGR_DATA_HW *tcamDataP)
{
	MSD_STATUS       retVal;    /* Functions return value */
	int i;
	...
	for(i=2; i<5; i++)
	{
		retVal = msdGetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, (MSD_U8)i, &tcamDataP->frame[i - 2]);
		if(retVal != MSD_OK)
		{
            msdRegisterAccessFlush(dev->devNum);
            return retVal;
...
static MSD_STATUS Willow_waitTcamReady(MSD_QD_DEV           *dev)
{
    MSD_STATUS       retVal;    /* Functions return value */
	...
	if (!IS_RMU_SUPPORTED(dev))
	{
		...
		while (data == 1)
		{
			retVal = msdGetAnyRegField(dev->devNum, WILLOW_TCAM_DEV_ADDR, WILLOW_QD_REG_TCAM_OPERATION, 15, 1, &data);
			if (retVal != MSD_OK)
			{
				return retVal;
			}
	...
	{
		if (Willow_msdWaitForBit(dev->devNum, WILLOW_TCAM_DEV_ADDR, WILLOW_QD_REG_TCAM_OPERATION, 15, 0) != MSD_OK)
		{
			msdRegisterAccessFlush(dev->devNum);
	...
	/* in RMU buffering case we may leave this function with the buffered wait command */
    return MSD_OK;
}

/*******************************************************************************
* Willow_tcamOperationPerform
*
* DESCRIPTION:
...
)
{
	...
	retVal = Willow_waitTcamReady(dev);
	if(retVal != MSD_OK)
	{
		msdRegisterAccessFlush(dev->devNum);
	...
	switch (tcamOp)
	{
		case Willow_TCAM_FLUSH_ALL:
		{
			data = 0;
			data = (MSD_U16)((1 << 15) | (tcamOp << 12));
			retVal = msdSetAnyReg(dev->devNum, WILLOW_TCAM_DEV_ADDR,WILLOW_QD_REG_TCAM_OPERATION,data);
			if(retVal != MSD_OK)
			{
				msdRegisterAccessFlush(dev->devNum);
				msdSemGive(dev->devNum,  dev->tblRegsSem);
		...
		case Willow_TCAM_LOAD_ENTRY:
		/*    case Willow_TCAM_PURGE_ENTRY: */
		{
			/* load Page 2 */

            retVal = Willow_tcamSetPage2Data(dev, &opData->tcamDataP);
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum,  dev->tblRegsSem);
				return retVal;
			...
			data = 0;
			data = (MSD_U16)((1 << 15) | (Willow_TCAM_LOAD_ENTRY << 12) | (2 << 10) | opData->tcamEntry);
			retVal = msdSetAnyReg(dev->devNum, WILLOW_TCAM_DEV_ADDR,WILLOW_QD_REG_TCAM_OPERATION,data);
			if(retVal != MSD_OK)
			{
				msdRegisterAccessFlush(dev->devNum);
			...
			retVal = Willow_tcamSetPage1Data(dev, &opData->tcamDataP);
			/* this will call the rmu buffer flush operation */
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum,  dev->tblRegsSem);
				return retVal;
			...
			data = 0;
			data = (MSD_U16)((1 << 15) | (Willow_TCAM_LOAD_ENTRY << 12) | (1 << 10) | opData->tcamEntry);
			retVal = msdSetAnyReg(dev->devNum, WILLOW_TCAM_DEV_ADDR,WILLOW_QD_REG_TCAM_OPERATION,data);
			if(retVal != MSD_OK)
			{
                msdRegisterAccessFlush(dev->devNum);
			...
			retVal = Willow_tcamSetPage0Data(dev, &opData->tcamDataP);
			/* this will execute the buffered RMU operations */
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum,  dev->tblRegsSem);
				return retVal;
			...
			data = 0;
			data = (MSD_U16)((1 << 15) | (Willow_TCAM_LOAD_ENTRY << 12) | (0 << 10) | opData->tcamEntry);
			retVal = msdSetAnyReg(dev->devNum, WILLOW_TCAM_DEV_ADDR,WILLOW_QD_REG_TCAM_OPERATION,data);
			if(retVal != MSD_OK)
			{
				msdRegisterAccessFlush(dev->devNum);
		...
		case Willow_TCAM_GET_NEXT_ENTRY:
		{
			data = 0;
			data = (MSD_U16)((1 << 15) | (tcamOp << 12) | (opData->tcamEntry)) ;
			retVal = msdSetAnyReg(dev->devNum, WILLOW_TCAM_DEV_ADDR,WILLOW_QD_REG_TCAM_OPERATION,data);
			...
			retVal = msdGetAnyReg(dev->devNum, WILLOW_TCAM_DEV_ADDR,WILLOW_QD_REG_TCAM_OPERATION, &data);
            if (retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->tblRegsSem);
                return retVal;
			...
			{
			    MSD_U16 data1;
			    retVal = msdGetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, WILLOW_QD_REG_TCAM_P0_KEYS_1, &data1);
                if (retVal != MSD_OK)
                {
                    msdSemGive(dev->devNum, dev->tblRegsSem);
                    return retVal;
		...
		/** fall through **/
		case Willow_TCAM_READ_ENTRY:
		{
			tcamOp = Willow_TCAM_READ_ENTRY;
			...
			retVal = Willow_tcamGetPage0Data(dev, &opData->tcamDataP);
			/* this will caus the RMU buffer to flush */
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
				return retVal;
			...
			retVal = Willow_tcamGetPage1Data(dev, &opData->tcamDataP);
			/* this will cause the RMU buffer to flush */
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
				return retVal;
			...
			retVal = Willow_tcamGetPage2Data(dev, &opData->tcamDataP);
			/* this will cause the RMU buffer to flush */
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
...
)
{
	...
	switch (tcamOp)
	{
		case Willow_TCAM_FLUSH_ENTRY:
		{
			retVal = msdSetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, WILLOW_QD_REG_TCAM_EGR_PORT, opData->port);
			if(retVal != MSD_OK)
			{
				msdRegisterAccessFlush(dev->devNum);
				msdSemGive(dev->devNum, dev->tblRegsSem);
			...
			retVal = msdSetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, WILLOW_QD_REG_TCAM_EGR_PORT, 0);
			if(retVal != MSD_OK)
			{
				msdRegisterAccessFlush(dev->devNum);
				msdSemGive(dev->devNum, dev->tblRegsSem);
		...
		case Willow_TCAM_LOAD_ENTRY:
		{
			...
			retVal = Willow_tcamSetPage3Data(dev, &opData->tcamDataP);
			/* this will force the RMU buffer to flush */
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
				return retVal;
			...
			data = 0;
			data = (MSD_U16)((1 << 15) | (Willow_TCAM_LOAD_ENTRY << 12) | (3 << 10) | (opData->tcamEntry));
			retVal = msdSetAnyReg(dev->devNum, WILLOW_TCAM_DEV_ADDR,WILLOW_QD_REG_TCAM_OPERATION,data);
			if(retVal != MSD_OK)
			{
				msdRegisterAccessFlush(dev->devNum);
		...
		case Willow_TCAM_GET_NEXT_ENTRY:
		{
			...
			if ((data&0x3f)==0x3f)
			{
	    		...
	    		retVal = msdGetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, WILLOW_QD_REG_TCAM_EGR_ACTION_1, &data1);
		    	if (retVal != MSD_OK)
    			{
	    			msdRegisterAccessFlush(dev->devNum);
    				msdSemGive(dev->devNum, dev->tblRegsSem);
    			...
    			retVal = msdGetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, WILLOW_QD_REG_TCAM_EGR_ACTION_2, &data2);
	    		if(retVal != MSD_OK)
		    	{
			    	msdRegisterAccessFlush(dev->devNum);
				    msdSemGive(dev->devNum, dev->tblRegsSem);
    			...
    			retVal = msdGetAnyReg(dev->devNum,  WILLOW_TCAM_DEV_ADDR, WILLOW_QD_REG_TCAM_EGR_ACTION_3, &data3);
			    if (retVal != MSD_OK)
			    {
                    msdRegisterAccessFlush(dev->devNum);
                    msdSemGive(dev->devNum, dev->tblRegsSem);
		...
		case Willow_TCAM_READ_ENTRY:
		{
			...
			retVal = Willow_tcamGetPage3Data(dev, &opData->tcamDataP);
			/* this will cause the RMU buffer to flush */
			if(retVal != MSD_OK)
			{
				msdSemGive(dev->devNum, dev->tblRegsSem);
