...
/*
 * Typedef: enum WILLOW_MSD_PIRL_OPERATION
 *
 * Description: Defines the PIRL (Port Ingress Rate Limit) Operation type
 *
 * Fields:
 *      Willow_PIRL_INIT_ALL_RESOURCE - Initialize all resources to the inital state
 *      Willow_PIRL_INIT_RESOURCE     - Initialize selected resources to the inital state
 *      Willow_PIRL_WRITE_RESOURCE    - Write to the selected resource/register
 *      Willow_PIRL_READ_RESOURCE     - Read from the selected resource/register
 */
typedef enum
{
    Willow_PIRL_INIT_ALL_RESOURCE      = 0x1,
    Willow_PIRL_INIT_RESOURCE          = 0x2,
    Willow_PIRL_WRITE_RESOURCE         = 0x3,
    Willow_PIRL_READ_RESOURCE          = 0x4
} WILLOW_MSD_PIRL_OPERATION;


/*
 *  typedef: struct WILLOW_MSD_PIRL_OP_DATA
 *
...
typedef struct
{
    ...
    MSD_U32    irlReg;
    MSD_U32    irlData;
} WILLOW_MSD_PIRL_OP_DATA;

/****************************************************************************/
/* STATS operation function declaration.                                    */
/****************************************************************************/
static MSD_STATUS Willow_pirlOperationPerform
(
    IN    MSD_QD_DEV           *dev,
    IN    WILLOW_MSD_PIRL_OPERATION   pirlOp,
    IN    MSD_U32              irlPort,
    IN    MSD_U32              irlRes,
    IN    MSD_U32              irlReg,
    INOUT WILLOW_MSD_PIRL_OP_DATA     *opData
);
static MSD_STATUS Willow_UpIntToConstant
(
	IN   MSD_U32 TimeUpInt,
	OUT  WILLOW_MSD_PIRL_DATA    *pirlData
 );

static MSD_STATUS Willow_ConstantToUpInt
(
 
	IN    WILLOW_MSD_PIRL_DATA    *pirlData,
...
*
*******************************************************************************/
MSD_STATUS Willow_gpirlInitialize
(
    IN  MSD_QD_DEV              *dev
)
{
    MSD_STATUS       retVal;    /* Functions return value */
    WILLOW_MSD_PIRL_OPERATION    op;

    MSD_DBG_INFO(("Willow_gpirlInitialize Called.\n"));

    op = Willow_PIRL_INIT_ALL_RESOURCE;

    retVal = Willow_pirlOperationPerform(dev, op, 0, 0, 0, NULL);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_pirlOperationPerform Willow_PIRL_INIT_ALL_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gpirlInitialize Exit.\n"));
    return retVal;
}
...
*
*******************************************************************************/
MSD_STATUS Willow_gpirlInitResource
(
    IN    MSD_QD_DEV            *dev,
...
)
{
    MSD_STATUS       retVal;    /* Functions return value */
    WILLOW_MSD_PIRL_OPERATION    op;
    WILLOW_MSD_PIRL_OP_DATA        opData;
	MSD_U8			port;

    MSD_DBG_INFO(("Willow_gpirlInitResource Called.\n"));

    op = Willow_PIRL_INIT_RESOURCE;
    ...
    retVal = Willow_pirlOperationPerform(dev, op, port, irlRes, 0, &opData);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_pirlOperationPerform Willow_PIRL_INIT_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gpirlInitResource Exit.\n"));
    return retVal;
}
...
*
*******************************************************************************/
MSD_STATUS Willow_gpirlCustomSetup
(
    IN  MSD_U32	   	tgtRate,
    IN  MSD_U32	   	tgtBstSize,
	INOUT WILLOW_MSD_PIRL_CUSTOM_RATE_LIMIT  *customSetup
)
{
    ...
    MSD_DBG_INFO(("Willow_gpirlCustomSetup Called.\n"));

	msdMemSet(&Setup,0,sizeof(WILLOW_MSD_PIRL_CUSTOM_RATE_LIMIT));
	...
	if(customSetup->countMode == WILLOW_MSD_PIRL_COUNT_FRAME)
		mode = 8.0;
    ...
    if (deltaMin == 1.0)
    {
        MSD_DBG_ERROR(("Failed (Willow_gpirlCustomSetup tune failed).\n"));
        return MSD_FAIL;
    }
    ...
    MSD_DBG_INFO(("Willow_gpirlCustomSetup Exit.\n"));
    return MSD_OK;	
#endif
}
...
*
*******************************************************************************/
MSD_STATUS Willow_gpirlReadResource
(
    IN  MSD_QD_DEV       *dev,
    IN  MSD_LPORT    	irlPort,
	IN  MSD_U32        	irlRes,
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gpirlReadResource Called.\n"));

    op = Willow_PIRL_READ_RESOURCE;
	port = MSD_LPORT_2_PORT(irlPort);
	if(port == MSD_INVALID_PORT)
	{
		MSD_DBG_ERROR(("Bad Port: %u.\n", (unsigned int)irlPort));
    ...
    retVal = Willow_pirlOperationPerform(dev, op, 0xf, irlRes, 1, &opData);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_pirlOperationPerform Willow_PIRL_READ_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

	msdMemSet((void*)(pirlData), 0, sizeof(WILLOW_MSD_PIRL_DATA));

	retVal = Willow_UpIntToConstant((opData.irlData & 0x7), pirlData);
	if(retVal != MSD_OK)
	{
        MSD_DBG_ERROR(("Willow_UpIntToConstantreturned: %s.\n", msdDisplayStatus(retVal)));
        return MSD_BAD_PARAM;
	}
    ...
    for(i=0; i<8; i++)
    {
        ...
        retVal = Willow_pirlOperationPerform(dev, op, port, irlRes, i, &opData);
        if (retVal != MSD_OK)
        {
            MSD_DBG_ERROR(("Willow_pirlOperationPerform Willow_PIRL_READ_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
            return retVal;
    ...
    /* reg6 data */
    pirlData->fcMode = ((data[6] >> 14) & 0x1)?WILLOW_MSD_PIRL_FC_DEASSERT_CBS_LIMIT:WILLOW_MSD_PIRL_FC_DEASSERT_EMPTY;
    pirlData->actionMode = ((data[6] >> 13) & 0x1)?Willow_PIRL_ACTION_ACCEPT:Willow_PIRL_ACTION_USE_LIMIT_ACTION;
    pirlData->ebsLimitAction = ((data[6] >> 12) & 0x1)?Willow_EBS_LIMIT_ACTION_FC:Willow_EBS_LIMIT_ACTION_DROP;
    pirlData->samplingMode = ((data[6] >> 11) & 0x1)?MSD_TRUE:MSD_FALSE;
    pirlData->accountQConf = ((data[6] >> 10) & 0x1)?MSD_TRUE:MSD_FALSE;
    pirlData->accountFiltered = ((data[6] >> 9) & 0x1)?MSD_TRUE:MSD_FALSE;
    pirlData->priORpt = ((data[6]>>8) & 0x1)?MSD_TRUE:MSD_FALSE;
	
	/* reg7 data */
    
	pirlData->priSelect = (MSD_U8)((data[7]>>8) & 0xff);
	pirlData->tcamFlows = ((data[7]>>3) & 0x1)?MSD_TRUE:MSD_FALSE;
	pirlData->mgntNrlEn = ((data[7] >> 2) & 0x1)?MSD_TRUE:MSD_FALSE;
        pirlData->saAVBNrlEn = ((data[7] >> 1) & 0x1)?MSD_TRUE:MSD_FALSE;
	pirlData->daAVBNrlEn = ((data[7]>>0) & 0x1)?MSD_TRUE:MSD_FALSE;

    MSD_DBG_INFO(("Willow_gpirlReadResource Exit.\n"));
    return MSD_OK;
}
...
*
*******************************************************************************/
MSD_STATUS Willow_gpirlWriteResource
(
    IN  MSD_QD_DEV       *dev,
    IN  MSD_LPORT    	irlPort,
	IN  MSD_U32        	irlRes,
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gpirlWriteResource Called.\n"));

    op = Willow_PIRL_WRITE_RESOURCE;
	port = MSD_LPORT_2_PORT(irlPort);
	if(port == MSD_INVALID_PORT)
	{
		MSD_DBG_ERROR(("Bad Port: %u.\n", (unsigned int)irlPort));
    ...
    data[6] = (MSD_U16)((pirlData->customSetup.ebsLimit >> 16)    |    /* Bit[7:0] : EBS Limit[23:16] */
                    ...
                    (((MSD_U16)pirlData->samplingMode?1:0) << 11)  | /* Bit[11] : sampling Mode */
                    (((MSD_U16)pirlData->accountQConf?1:0) << 10)   | /* Bit[10] : account discarded frames due to queue congestion */
                    (((MSD_U16)pirlData->accountFiltered) << 9)    |    /* Bit[9] : account filtered frames. Account for All  */
                    (((MSD_U16)pirlData->priORpt?0x1:0) << 8));    /* Bit[8] : typeMask OR priMask */
			
	/* reg7 data */
    data[7] = (MSD_U16)((((MSD_U16)pirlData->priSelect) << 8)    |    /* Bit[15:8] : priority bit mask */
                    (((MSD_U16)pirlData->tcamFlows?1:0) << 3)  | /* Bit[3] : tcam flows account enable bit*/
                    (((MSD_U16)pirlData->mgntNrlEn?1:0) << 2)  |  /* Bit[2] : management Non Rate limit enable bit*/
                    (((MSD_U16)pirlData->saAVBNrlEn?1:0) << 1)  |  /* Bit[1] : SA AVB Non Rate limit enable bit*/
                    (((MSD_U16)pirlData->daAVBNrlEn?1:0) << 0));  /* Bit[0] : DA AVB Non Rate limit enable bit*/
                    
	retVal = Willow_ConstantToUpInt(pirlData, &opData.irlData);
	if(retVal != MSD_OK)
	{
    ...
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_pirlOperationPerform Willow_PIRL_WRITE_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    MSD_DBG_INFO(("Willow_gpirlWriteResource Exit.\n"));
    return MSD_OK;  
}
...
*
*******************************************************************************/
MSD_STATUS Willow_gpirlGetResReg
(
    IN  MSD_QD_DEV    *dev,
    IN  MSD_LPORT	 irlPort,
	IN  MSD_U32		 irlRes,
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gpirlGetResReg Called.\n"));
	...
	retVal = Willow_pirlOperationPerform(dev, op, port, irlRes, regOffset, &opData);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_pirlOperationPerform Willow_PIRL_READ_RESOURCE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
	...
	*data = (MSD_U16)opData.irlData;
    ...
    MSD_DBG_INFO(("Willow_gpirlGetResReg Exit.\n"));
    return MSD_OK;
}
...
*
*******************************************************************************/
MSD_STATUS Willow_gpirlSetResReg
(
    IN  MSD_QD_DEV    *dev,
    IN  MSD_LPORT	 irlPort,
	IN  MSD_U32		 irlRes,
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gpirlSetResReg Called.\n"));
    ...
    MSD_DBG_INFO(("Willow_gpirlSetResReg Exit.\n"));
    return MSD_OK;  
}
...
)
{
    ...
    while(data == 1)
    {
        retVal = msdGetAnyRegField(dev->devNum,  WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_IGR_RATE_COMMAND,15,1,&data);
        if(retVal != MSD_OK)
        {
    ...
    switch (pirlOp)
    {
        case Willow_PIRL_INIT_ALL_RESOURCE:
            data = (1 << 15) | (Willow_PIRL_INIT_ALL_RESOURCE << 12);
            retVal = msdSetAnyReg(dev->devNum,  WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_IGR_RATE_COMMAND,data);
            if(retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->pirlRegsSem);
                return retVal;
            ...
            break;
        case Willow_PIRL_INIT_RESOURCE:
            data = (MSD_U16)((1 << 15) | (Willow_PIRL_INIT_RESOURCE << 12) | (irlPort&0xf)<<8 | 
                    ((irlRes&0x7)<< 5));
            retVal = msdSetAnyReg(dev->devNum,  WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_IGR_RATE_COMMAND,data);
            if(retVal != MSD_OK)
            {
        ...
        case Willow_PIRL_WRITE_RESOURCE:
            data = (MSD_U16)opData->irlData;
            retVal = msdSetAnyReg(dev->devNum,  WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_IGR_RATE_DATA,data);
            if(retVal != MSD_OK)
            {
                msdSemGive(dev->devNum, dev->pirlRegsSem);
                return retVal;
            ...
            data = (MSD_U16)((1 << 15) | (Willow_PIRL_WRITE_RESOURCE << 12) | (irlPort&0xf)<<8 | 
		...
		case Willow_PIRL_READ_RESOURCE:
			data = (MSD_U16)((1 << 15) | (Willow_PIRL_READ_RESOURCE << 12) | (irlPort & 0xf) << 8 |
                    ((irlRes&0x7) << 5) | (irlReg & 0xF));
            retVal = msdSetAnyReg(dev->devNum,  WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_IGR_RATE_COMMAND,data);
            if(retVal != MSD_OK)
            {
            ...
            retVal = msdGetAnyReg(dev->devNum,  WILLOW_GLOBAL2_DEV_ADDR,WILLOW_QD_REG_IGR_RATE_DATA,&data);
