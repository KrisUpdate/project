...
typedef struct _WILLOW_MSD_EXTRA_OP_DATA
{
    MSD_U32	moveFrom;
    MSD_U32	moveTo;
    MSD_U32	intCause;
    MSD_U32	reserved;
} WILLOW_MSD_EXTRA_OP_DATA;
...
typedef enum
{
    WILLOW_MSD_ATU_STATS_ALL = 0,
    WILLOW_MSD_ATU_STATS_NON_STATIC,
    WILLOW_MSD_ATU_STATS_ALL_FID,
    WILLOW_MSD_ATU_STATS_NON_STATIC_FID
} WILLOW_MSD_ATU_STATS_OP;
...
/****************************************************************************/
static MSD_STATUS Willow_atuOperationPerform
(
    IN      MSD_QD_DEV           *dev,
	IN      WILLOW_MSD_ATU_OPERATION    atuOp,
	INOUT   WILLOW_MSD_EXTRA_OP_DATA    *opData,
    INOUT   WILLOW_MSD_ATU_ENTRY        *atuEntry
);

static MSD_STATUS Willow_atuGetStats
(
    IN  MSD_QD_DEV    *dev,
    IN  WILLOW_MSD_ATU_STAT    *atuStat,
    OUT MSD_U32        *count
);

/*******************************************************************************
* Willow_gfdbGetAtuEntryNext
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbGetAtuEntryNext
(
    IN MSD_QD_DEV    *dev,
    INOUT WILLOW_MSD_ATU_ENTRY  *atuEntry
)
{
    MSD_STATUS       retVal;
    WILLOW_MSD_ATU_ENTRY    entry;

    MSD_DBG_INFO(("Willow_gfdbGetAtuEntryNext Called.\n"));
    ...
    retVal = Willow_atuOperationPerform(dev,WILLOW_GET_NEXT_ENTRY,NULL,&entry);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Willow_atuOperationPerform WILLOW_GET_NEXT_ENTRY returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    if(WILLOW_IS_BROADCAST_MAC(entry.macAddr))
    {
        if(entry.entryState == 0)
        {
            MSD_DBG_INFO(("No more valid Entry found!.\n"));
            MSD_DBG_INFO(("Willow_gfdbGetAtuEntryNext Exit.\n"));
            return MSD_NO_SUCH;
        }
    }
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbFlush
(
    IN MSD_QD_DEV    *dev,
    IN WILLOW_MSD_FLUSH_CMD flushCmd
)
{
    MSD_STATUS       retVal;
    WILLOW_MSD_ATU_ENTRY    entry;

    MSD_DBG_INFO(("Willow_gfdbFlush Called.\n"));

    entry.DBNum = 0;
    entry.entryState = 0;

    if(flushCmd == WILLOW_MSD_FLUSH_ALL)
        retVal = Willow_atuOperationPerform(dev,WILLOW_FLUSH_ALL,NULL,&entry);
    else if (flushCmd == WILLOW_MSD_FLUSH_ALL_NONSTATIC)
        retVal = Willow_atuOperationPerform(dev,WILLOW_FLUSH_NONSTATIC,NULL,&entry);
    else
    {
        MSD_DBG_ERROR(("Bad Param: WILLOW_MSD_FLUSH_CMD is %d\n", flushCmd));
        return MSD_FAIL;
    }

    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Willow_atuOperationPerform WILLOW_FLUSH returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gfdbFlush Exit.\n"));
    return MSD_OK;
}


/*******************************************************************************
* Willow_gfdbFlushInDB
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbFlushInDB
(
    IN MSD_QD_DEV    *dev,
    IN WILLOW_MSD_FLUSH_CMD flushCmd,
    IN MSD_U32 DBNum
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbFlushInDB Called.\n"));
    ...
    if(flushCmd == WILLOW_MSD_FLUSH_ALL)
        retVal = Willow_atuOperationPerform(dev,WILLOW_FLUSH_ALL_IN_DB,NULL,&entry);
    else if (flushCmd == WILLOW_MSD_FLUSH_ALL_NONSTATIC)
        retVal = Willow_atuOperationPerform(dev,WILLOW_FLUSH_NONSTATIC_IN_DB,NULL,&entry);
    else
    {
        MSD_DBG_ERROR(("Bad Param: WILLOW_MSD_FLUSH_CMD is %d\n", flushCmd));
        return MSD_FAIL;
    ...
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Willow_atuOperationPerform WILLOW_FLUSH_IN_DB returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

	MSD_DBG_INFO(("Willow_gfdbFlushInDB Exit.\n"));
    return MSD_OK;
}
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbMove
(
    IN MSD_QD_DEV    *dev,
    IN WILLOW_MSD_MOVE_CMD  moveCmd,
...
)
{
    MSD_STATUS       retVal;
    WILLOW_MSD_ATU_ENTRY    entry;
    WILLOW_MSD_EXTRA_OP_DATA    opData;

    MSD_DBG_INFO(("Willow_gfdbMove Called.\n"));
    ...
    if(moveCmd == WILLOW_MSD_MOVE_ALL)
        retVal = Willow_atuOperationPerform(dev,WILLOW_FLUSH_ALL,&opData,&entry);
    else if (moveCmd == WILLOW_MSD_MOVE_ALL_NONSTATIC)
        retVal = Willow_atuOperationPerform(dev,WILLOW_FLUSH_NONSTATIC,&opData,&entry);
    else
    {
        MSD_DBG_ERROR(("Bad Param: WILLOW_MSD_MOVE_CMD is %d\n", moveCmd));
        return MSD_FAIL;
    }

    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Willow_atuOperationPerform WILLOW_MOVE returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gfdbMove Exit.\n"));
    return MSD_OK;
}
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbMoveInDB
(
    IN MSD_QD_DEV     *dev,
    IN WILLOW_MSD_MOVE_CMD   moveCmd,
    IN MSD_U32        DBNum,
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbMoveInDB Called.\n"));
    ...
    if(moveCmd == WILLOW_MSD_MOVE_ALL)
        retVal = Willow_atuOperationPerform(dev,WILLOW_FLUSH_ALL_IN_DB,&opData,&entry);
    else if (moveCmd == WILLOW_MSD_MOVE_ALL_NONSTATIC)
        retVal = Willow_atuOperationPerform(dev,WILLOW_FLUSH_NONSTATIC_IN_DB,&opData,&entry);
    else
    {
        MSD_DBG_ERROR(("Bad Param: WILLOW_MSD_MOVE_CMD is %d\n", moveCmd));
        return MSD_FAIL;
    ...
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Willow_atuOperationPerform WILLOW_MOVE_IN_DB returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gfdbMoveInDB Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbAddMacEntry
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbAddMacEntry
(
    IN MSD_QD_DEV    *dev,
    IN WILLOW_MSD_ATU_ENTRY *macEntry
)
{
    MSD_STATUS       retVal;
    WILLOW_MSD_ATU_ENTRY    entry;

    MSD_DBG_INFO(("Willow_gfdbAddMacEntry Called.\n"));
    ...
    if (MSD_TRUE == macEntry->LAG && 0 == WILLOW_IS_TRUNK_ID_VALID(dev, macEntry->portVec))
    {
    ...
    retVal = Willow_atuOperationPerform(dev,WILLOW_LOAD_PURGE_ENTRY,NULL,&entry);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Willow_atuOperationPerform WILLOW_LOAD_PURGE_ENTRY returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gfdbAddMacEntry Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbDelAtuEntry
*
* DESCRIPTION:
*       Deletes ATU entry.
*
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbDelAtuEntry
(
    IN MSD_QD_DEV     *dev,
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbDelAtuEntry Called.\n"));

    if (NULL == macAddr)
    {
        MSD_DBG_ERROR(("Input param MSD_ETHERADDR in Willow_gfdbDelAtuEntry is NULL. \n"));
        return MSD_BAD_PARAM;
    }
    ...
    MSD_DBG_INFO(("Willow_gfdbDelAtuEntry Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbGetViolation
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbGetViolation
(
    IN  MSD_QD_DEV         *dev,
    OUT WILLOW_MSD_ATU_INT_STATUS *atuIntStatus
)
{
    MSD_U16              intCause;
    MSD_STATUS           retVal;
    ...
    MSD_DBG_INFO(("Willow_gfdbGetViolation Called.\n"));

    /* check which Violation occurred */
    retVal = msdGetAnyRegField(dev->devNum,  WILLOW_GLOBAL1_DEV_ADDR,WILLOW_QD_REG_GLOBAL_STATUS,3,1,&intCause);
    if(retVal != MSD_OK)
    {
    ...
    if (!intCause)
    {
		...
		MSD_DBG_INFO(("Willow_gfdbGetViolation Exit, No Violation occurred.\n"));        
        return MSD_OK;
    }

    entry.DBNum = 0;

    retVal = Willow_atuOperationPerform(dev,WILLOW_SERVICE_VIOLATIONS,&opData,&entry);
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Willow_atuOperationPerform WILLOW_SERVICE_VIOLATIONS returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    MSD_DBG_INFO(("Willow_gfdbGetViolation Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbFindAtuMacEntry
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbFindAtuMacEntry
(
    IN MSD_QD_DEV    *dev,
    INOUT WILLOW_MSD_ATU_ENTRY  *atuEntry,
    OUT MSD_BOOL         *found
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbFindAtuMacEntry Called.\n"));
    ...
    *found = MSD_FALSE;
    ...
    retVal = Willow_gfdbGetAtuEntryNext(dev,&entry);
    if (MSD_NO_SUCH == retVal)
    {
        MSD_DBG_INFO(("Willow_gfdbFindAtuMacEntry Exit, Not found valid entry.\n"));
        return MSD_OK;
    }
    if(retVal != MSD_OK)
    {
		MSD_DBG_ERROR(("Willow_gfdbGetAtuEntryNext returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    {
		MSD_DBG_INFO(("Willow_gfdbFindAtuMacEntry Exit, Not found this entry.\n"));
        return MSD_OK;
    }
    ...
    *found = MSD_TRUE;
    MSD_DBG_INFO(("Willow_gfdbFindAtuMacEntry Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbSetAgingTimeout
*
* DESCRIPTION:
...
*        For willow, the time-base is 3750 milliseconds, so it can be set to 3750,
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbSetAgingTimeout
(
    IN MSD_QD_DEV    *dev,
    IN MSD_U32 timeout
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbSetAgingTimeout Called.\n"));
    ...
    retVal = msdSetAnyRegField(dev->devNum, WILLOW_GLOBAL1_DEV_ADDR,WILLOW_QD_REG_ATU_CTRL_REG,4,8,data);
    /* msdSetAnyRegField currently forces a flush */
    /* msdRegisterAccessFlush(devNum); */
    if(retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Willow_gfdbSetAgingTimeout Exit.\n"));
    return MSD_OK;
}

/* Begin New Add */
/*******************************************************************************
* Willow_gfdbGetAgingTimeout
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbGetAgingTimeout
(
    IN  MSD_QD_DEV  *dev,
    IN  MSD_U32  *timeout
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbGetAgingTimeout Called.\n"));

    timeBase = 3750;

    /* Set the Time Out value.              */
    retVal = msdGetAnyRegField(dev->devNum, WILLOW_GLOBAL1_DEV_ADDR, WILLOW_QD_REG_ATU_CTRL_REG, 4, 8, &data);
    /* msdSetAnyRegField currently forces a flush */
    /* msdRegisterAccessFlush(devNum); */
    if (retVal != MSD_OK)
    {
    ...
    *timeout = (MSD_U32)(data*timeBase);
    ...
    MSD_DBG_INFO(("Willow_gfdbGetAgingTimeout Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbGetLearn2All
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbGetLearn2All
(
    IN  MSD_QD_DEV  *dev,
    OUT MSD_BOOL  *mode
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbGetLearn2All Called.\n"));

    /* Get the Learn2All. */
    data = 0;
    retVal = msdGetAnyRegField(dev->devNum, WILLOW_GLOBAL1_DEV_ADDR, WILLOW_QD_REG_ATU_CTRL_REG, 3, 1, &data);
    /* msdSetAnyRegField currently forces a flush */
    /* msdRegisterAccessFlush(devNum); */
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdGetAnyRegField for WILLOW_QD_REG_ATU_CTRL_REG returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_BIT_2_BOOL(data, *mode);

    MSD_DBG_INFO(("Willow_gfdbGetLearn2All Exit.\n"));
    return retVal;
}

/*******************************************************************************
* Willow_gfdbSetLearn2All
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbSetLearn2All
(
    IN  MSD_QD_DEV  *dev,
    IN  MSD_BOOL  mode
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbSetLearn2All Called.\n"));

    /* Set the Learn2All. */
    MSD_BOOL_2_BIT(mode, data);
    retVal = msdSetAnyRegField(dev->devNum, WILLOW_GLOBAL1_DEV_ADDR, WILLOW_QD_REG_ATU_CTRL_REG, 3, 1, data);
    /* msdSetAnyRegField currently forces a flush */
    /* msdRegisterAccessFlush(devNum); */
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdSetAnyRegField for WILLOW_QD_REG_ATU_CTRL_REG returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gfdbSetLearn2All Exit.\n"));
    return retVal;
}

/*******************************************************************************
* Willow_gfdbSetPortLearnLimit
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbSetPortLearnLimit
(
    IN  MSD_QD_DEV  *dev,
...
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbSetPortLearnLimit Called.\n"));
    ...
    if (limit > WILLOW_MAX_ATU_PORT_LEARNLIMIT)
    {
        MSD_DBG_ERROR(("Learn Limit %u our of range. It should be within [0, %u].\n", (unsigned int)limit, WILLOW_MAX_ATU_PORT_LEARNLIMIT));
        return MSD_BAD_PARAM;
    }
    ...
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdGetAnyRegField for WILLOW_QD_REG_PORT_CONTROL1 returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    retVal = msdGetAnyRegField(dev->devNum, phyAddr, WILLOW_QD_REG_PAV, 0, 11, &data);
    /* msdSetAnyRegField currently forces a flush */
    /* msdRegisterAccessFlush(devNum); */
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdGetAnyRegField for WILLOW_QD_REG_PAV returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    isPavMBsSet = 0;
    while (data != 0)
    ...
    retVal = msdGetAnyReg(dev->devNum, phyAddr, WILLOW_QD_REG_PORT_ATU_CONTROL, &data);
    retVal2 = msdRegisterAccessFlush(dev->devNum);
    if ((retVal != MSD_OK) || (retVal2 != MSD_OK))
    {
        if (retVal == MSD_OK)
            retVal = retVal2; 
        MSD_DBG_ERROR(("msdGetAnyReg for WILLOW_QD_REG_PORT_ATU_CONTROL returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    ...
    data &= 0x6000;
    data |= limit;
    retVal = msdSetAnyReg(dev->devNum, phyAddr, WILLOW_QD_REG_PORT_ATU_CONTROL, data);
    if (retVal != MSD_OK)
    {
        msdRegisterAccessFlush(dev->devNum);
        MSD_DBG_ERROR(("msdSetAnyReg for WILLOW_QD_REG_PORT_ATU_CONTROL returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    /* Set back KeepOldLearnLimit */
    retVal = msdSetAnyRegField(dev->devNum, phyAddr, WILLOW_QD_REG_PORT_ATU_CONTROL, 12, 1, (MSD_U16)isKeepOldLearnLimit);
    /* msdSetAnyRegField currently forces a flush */
    /* msdRegisterAccessFlush(devNum); */
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdSetAnyRegField for WILLOW_QD_REG_PORT_ATU_CONTROL returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gfdbSetPortLearnLimit Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbGetPortLearnLimit
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbGetPortLearnLimit
(
    IN  MSD_QD_DEV  *dev,
    IN  MSD_LPORT  portNum,
    OUT MSD_U32  *limit
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbGetPortLearnLimit Called.\n"));
    ...
    /* Get back limit*/
    retVal = msdGetAnyRegField(dev->devNum, phyAddr, WILLOW_QD_REG_PORT_ATU_CONTROL, 0, 10, &data);
    /* msdGetAnyRegField currently forces a flush */
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("msdGetAnyRegField for WILLOW_QD_REG_PORT_ATU_CONTROL returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }
    *limit = data;
    ...
    MSD_DBG_INFO(("Willow_gfdbGetPortLearnLimit Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbGetPortLearnCount
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbGetPortLearnCount
(
    IN  MSD_QD_DEV   *dev,
    IN  MSD_LPORT  portNum,
    IN  MSD_U32  *count
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbGetPortLearnCount Called.\n"));
	...
	retVal = msdSetAnyRegField(dev->devNum, phyAddr, WILLOW_QD_REG_PORT_ATU_CONTROL, 15, 1, 1);
    /* msdSetAnyRegField currently forces a flush */
    /* msdRegisterAccessFlush(devNum); */
	if (retVal != MSD_OK)
	{
    ...
    *count = data;
    ...
    MSD_DBG_INFO(("Willow_gfdbGetPortLearnCount Exit.\n"));
    return MSD_OK;
}


/*******************************************************************************
* Willow_gfdbGetEntryCount
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbGetEntryCount
(
    IN  MSD_QD_DEV  *dev,
    OUT MSD_U32  *count
)
{
    MSD_STATUS       retVal;
    WILLOW_MSD_ATU_STAT        atuStat;

    MSD_DBG_INFO(("Willow_gfdbGetEntryCount Called.\n"));

    atuStat.op = WILLOW_MSD_ATU_STATS_ALL;
    retVal = Willow_atuGetStats(dev, &atuStat, count);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_atuGetStats returned: %s.\n", msdDisplayStatus(retVal)));
        return retVal;
    }

    MSD_DBG_INFO(("Willow_gfdbGetEntryCount Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbGetEntryCountPerFid
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbGetEntryCountPerFid
(
    IN  MSD_QD_DEV  *dev,
    IN  MSD_U32  fid,
    OUT MSD_U32  *count
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbGetEntryCountPerFid Called.\n"));

    atuStat.op = WILLOW_MSD_ATU_STATS_ALL_FID;
    atuStat.DBNum = fid;
    retVal = Willow_atuGetStats(dev, &atuStat, count);
    if (retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Willow_gfdbGetEntryCountPerFid Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbGetNonStaticEntryCount
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbGetNonStaticEntryCount
(
    IN  MSD_QD_DEV  *dev,
    OUT MSD_U32  *count
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbGetNonStaticEntryCount Called.\n"));

    atuStat.op = WILLOW_MSD_ATU_STATS_NON_STATIC;
    retVal = Willow_atuGetStats(dev, &atuStat, count);
    if (retVal != MSD_OK)
    {
        MSD_DBG_ERROR(("Willow_atuGetStats returned: %s.\n", msdDisplayStatus(retVal)));
    ...
    MSD_DBG_INFO(("Willow_gfdbGetNonStaticEntryCount Exit.\n"));
    return MSD_OK;
}

/*******************************************************************************
* Willow_gfdbGetNonStaticEntryCountPerFid
*
* DESCRIPTION:
...
*
*******************************************************************************/
MSD_STATUS Willow_gfdbGetNonStaticEntryCountPerFid
(
    IN  MSD_QD_DEV  *dev,
    IN  MSD_U32  fid,
    OUT MSD_U32  *count
)
{
    ...
    MSD_DBG_INFO(("Willow_gfdbGetNonStaticEntryCountPerFid Called.\n"));

    atuStat.op = WILLOW_MSD_ATU_STATS_NON_STATIC_FID;
    atuStat.DBNum = fid;
    retVal = Willow_atuGetStats(dev, &atuStat, count);
    if (retVal != MSD_OK)
    {
    ...
    MSD_DBG_INFO(("Willow_gfdbGetNonStaticEntryCountPerFid Exit.\n"));
    return MSD_OK;
}
...
(
    ...
    IN      WILLOW_MSD_ATU_OPERATION    atuOp,
    INOUT    WILLOW_MSD_EXTRA_OP_DATA    *opData,
    INOUT     WILLOW_MSD_ATU_ENTRY        *entry
)
{
    MSD_STATUS       retVal;         /* Functions return value.      */
    MSD_STATUS       retVal2;        /* internal return value.      */
    MSD_U16          data, data2;    /* Data to be set into the      */
                                    /* register.                    */
    ...
    retVal = Willow_msdWaitForBit(dev->devNum, WILLOW_GLOBAL1_DEV_ADDR, WILLOW_QD_REG_ATU_OPERATION, 15, 0);
    if (retVal != MSD_OK)
    {
        msdRegisterAccessFlush(dev->devNum);
    ...
    switch (atuOp)
    {
        case WILLOW_LOAD_PURGE_ENTRY:
			...
			retVal = msdSetAnyReg(dev->devNum,  WILLOW_GLOBAL1_DEV_ADDR,WILLOW_QD_REG_ATU_DATA_REG,data);
			if(retVal != MSD_OK)
			{
				msdRegisterAccessFlush(dev->devNum);
				msdSemGive(dev->devNum, dev->atuRegsSem);
        ...
        case WILLOW_GET_NEXT_ENTRY:
			for(i = 0; i < 3; i++)
			{
				data=(entry->macAddr.arEther[2*i] << 8)|(entry->macAddr.arEther[1 + 2*i]);
				retVal = msdSetAnyReg(dev->devNum,  WILLOW_GLOBAL1_DEV_ADDR,(MSD_U8)(WILLOW_QD_REG_ATU_MAC_BASE+i),data);
				if(retVal != MSD_OK)
				{
					msdRegisterAccessFlush(dev->devNum);
					msdSemGive(dev->devNum, dev->atuRegsSem);
        ...
        case WILLOW_FLUSH_ALL:
        case WILLOW_FLUSH_NONSTATIC:
        case WILLOW_FLUSH_ALL_IN_DB:
        case WILLOW_FLUSH_NONSTATIC_IN_DB:
        ...
        case WILLOW_SERVICE_VIOLATIONS:
			break;

        default:
			/* it actually doesn't make sense to send out the Wait when we don't
			   have anything to do but recalling it might be difficult too */
			msdRegisterAccessFlush(dev->devNum);
			msdSemGive(dev->devNum, dev->atuRegsSem);
            return MSD_FAIL;
    }
    ...
    /* Set DBNum */
	retVal = msdSetAnyRegField(dev->devNum,  WILLOW_GLOBAL1_DEV_ADDR, WILLOW_QD_REG_ATU_FID_REG, 0, 12, (MSD_U16)(entry->DBNum & 0xFFF));
	/* msdGetAnyRegField currently forces a flush */
	if (retVal != MSD_OK)
	{
		msdSemGive(dev->devNum, dev->atuRegsSem);
	...
	retVal = msdGetAnyReg(dev->devNum,  WILLOW_GLOBAL1_DEV_ADDR,WILLOW_QD_REG_ATU_OPERATION,&data);
	retVal2 = msdRegisterAccessFlush(dev->devNum);
    if ((retVal != MSD_OK) || (retVal2 != MSD_OK))
    {
        if (retVal == MSD_OK)
	...
	data &= 0x0fff;
	if(atuOp == WILLOW_LOAD_PURGE_ENTRY)
	{
		data &= 0x0f8;
	}
	opcodeData |= ((1 << 15) | (atuOp << 12) | data);

    retVal = msdSetAnyReg(dev->devNum,  WILLOW_GLOBAL1_DEV_ADDR,WILLOW_QD_REG_ATU_OPERATION,opcodeData);
    if(retVal != MSD_OK)
    {
        msdRegisterAccessFlush(dev->devNum);
        msdSemGive(dev->devNum, dev->atuRegsSem);
    ...
    if(atuOp == WILLOW_SERVICE_VIOLATIONS)
    {
        retVal = Willow_msdWaitForBit(dev->devNum, WILLOW_GLOBAL1_DEV_ADDR, WILLOW_QD_REG_ATU_OPERATION, 15, 0);
        if (retVal != MSD_OK)
        {
        ...
        /* Willow_get the Interrupt Cause */
        retVal = msdGetAnyRegField(dev->devNum,  WILLOW_GLOBAL1_DEV_ADDR,WILLOW_QD_REG_ATU_OPERATION,4,4,&data);
        /* msdGetAnyRegField currently forces a flush */
        if(retVal != MSD_OK)
        {
            msdSemGive(dev->devNum, dev->atuRegsSem);
        ...
        switch (data)
        {
            case 8:    /* Age Interrupt */
                opData->intCause = WILLOW_MSD_ATU_AGE_OUT_VIOLATION;
                break;
            case 4:    /* Member Violation */
                opData->intCause = WILLOW_MSD_ATU_MEMBER_VIOLATION;
                break;
            case 2:    /* Miss Violation */
                opData->intCause = WILLOW_MSD_ATU_MISS_VIOLATION;
                break;
            case 1:    /* Full Violation */
                opData->intCause = WILLOW_MSD_ATU_FULL_VIOLATION;
                break;
            default:
                opData->intCause = 0;
                msdSemGive(dev->devNum, dev->atuRegsSem);
        ...
        /* Willow_get the DBNum that was involved in the violation */

        entry->DBNum = 0;
		
		retVal = msdGetAnyRegField(dev->devNum,  WILLOW_GLOBAL1_DEV_ADDR, WILLOW_QD_REG_ATU_FID_REG, 0, 12, &data);
		/* msdGetAnyRegField currently forces a flush */
		if (retVal != MSD_OK)
		{
			msdSemGive(dev->devNum, dev->atuRegsSem);
        ...
        /* Willow_get the Source Port ID that was involved in the violation */

        retVal = msdGetAnyReg(dev->devNum,  WILLOW_GLOBAL1_DEV_ADDR,WILLOW_QD_REG_ATU_DATA_REG,&data);
        if (retVal != MSD_OK)
        {
            msdRegisterAccessFlush(dev->devNum);
            msdSemGive(dev->devNum, dev->atuRegsSem);
        ...
        for (i = 0; i < 3; i++)
        {
            retVal = msdGetAnyReg(dev->devNum, WILLOW_GLOBAL1_DEV_ADDR, (MSD_U8)(WILLOW_QD_REG_ATU_MAC_BASE + i), &macdata[i]);
            if (retVal != MSD_OK)
            {
                msdRegisterAccessFlush(dev->devNum);
                msdSemGive(dev->devNum, dev->atuRegsSem);
        ...
        for (i = 0; i < 3; i++)
        {
            entry->macAddr.arEther[2 * i] = (MSD_U8)((macdata[i] >> 8) & 0x00FF);
            entry->macAddr.arEther[1 + 2*i] = (MSD_U8)(macdata[i] & 0xFF);
        }
    } /* end of service violations */

    /* If the operation is a gen next operation wait for the response   */
    if(atuOp == WILLOW_GET_NEXT_ENTRY)
    {
        entry->LAG = MSD_FALSE;
        entry->exPrio.macFPri = 0;
        entry->exPrio.macQPri = 0;
        ...
        retVal = msdGetAnyRegField(dev->devNum, WILLOW_GLOBAL1_DEV_ADDR, WILLOW_QD_REG_ATU_OPERATION, 0, 11, &data2);
        /* msdGetAnyRegField currently forces a flush */
        if (retVal != MSD_OK)
        {
            msdSemGive(dev->devNum, dev->atuRegsSem);
        ...
        entry->exPrio.macFPri = (MSD_U8)(data2 & 0x7);
        entry->exPrio.macQPri = (MSD_U8)((data2>>8) & 0x7);
    }
...
)
{
    ...
    switch (atuStat->op)
    {
        case WILLOW_MSD_ATU_STATS_ALL:
        case WILLOW_MSD_ATU_STATS_NON_STATIC:
                dbNum = 0;
                break;
        case WILLOW_MSD_ATU_STATS_ALL_FID:
        case WILLOW_MSD_ATU_STATS_NON_STATIC_FID:
                dbNum = atuStat->DBNum;
                break;
        default:
            return MSD_FAIL;
    ...
    for (bin = 0; bin<4; bin++)
    {
        ...
        retVal = msdSetAnyReg(dev->devNum, WILLOW_GLOBAL2_DEV_ADDR, WILLOW_QD_REG_ATU_STATS, data);
        if (retVal != MSD_OK)
            return retVal;
        retVal = msdRegisterAccessFlush(dev->devNum);
        if (retVal != MSD_OK)
        ...
        retVal = msdSetAnyRegField(dev->devNum, WILLOW_GLOBAL2_DEV_ADDR, WILLOW_QD_REG_ATU_STATS, 14, 2, bin);
        /* msdGetAnyRegField currently forces a flush */
        if (retVal != MSD_OK)
        {
            return retVal;
        ...
        retVal = msdGetAnyRegField(dev->devNum, WILLOW_GLOBAL2_DEV_ADDR, WILLOW_QD_REG_ATU_STATS, 0, 12, &data);
        /* msdGetAnyRegField currently forces a flush */
        if (retVal != MSD_OK)
        {
            return retVal;
