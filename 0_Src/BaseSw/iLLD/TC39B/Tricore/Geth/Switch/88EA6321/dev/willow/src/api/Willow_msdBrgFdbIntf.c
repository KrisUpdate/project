...
)
{
	...
	if (NULL == macEntry)
	{
		MSD_DBG_ERROR(("Input param MSD_ATU_ENTRY in Willow_gfdbAddMacEntryIntf is NULL. \n"));
    ...
    return Willow_gfdbAddMacEntry(dev, &entry);
}
...
)
{
	...
	if (NULL == atuEntry)
	{
		MSD_DBG_ERROR(("Input param MSD_ATU_ENTRY in Willow_gfdbGetAtuEntryNextIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
...
{
	return Willow_gfdbFlush(dev, (WILLOW_MSD_FLUSH_CMD)flushCmd);
}
...
{
	return Willow_gfdbFlushInDB(dev, (WILLOW_MSD_FLUSH_CMD)flushCmd, DBNum);
}
...
{
	return Willow_gfdbMove(dev, (WILLOW_MSD_MOVE_CMD)moveCmd, moveFrom, moveTo);
}
...
{
	return Willow_gfdbMoveInDB(dev, (WILLOW_MSD_MOVE_CMD)moveCmd, DBNum, moveFrom, moveTo);
}
...
{
    WILLOW_MSD_MOVE_CMD pmoveCmd;

    switch (moveCmd)
    {
    case MSD_MOVE_ALL:
        pmoveCmd = WILLOW_MSD_MOVE_ALL;
        break;
    case MSD_MOVE_ALL_NONSTATIC:
        pmoveCmd = WILLOW_MSD_MOVE_ALL_NONSTATIC;
        break;
    default:
        return MSD_BAD_PARAM;
    }

    return Willow_gfdbMove(dev, pmoveCmd, portNum, 0x1F);
}
...
)
{
    ...
    return Willow_gfdbMoveInDB(dev, pmoveCmd, DBNum, portNum, 0x1F);
}
...
{
	WILLOW_MSD_ATU_INT_STATUS atuInt;
	MSD_STATUS       retVal;

	msdMemSet((void*)(&atuInt), 0, sizeof(WILLOW_MSD_ATU_INT_STATUS));
	retVal = Willow_gfdbGetViolation(dev, &atuInt);
	if (MSD_OK != retVal)
		return retVal;

	if (NULL == atuIntStatus)
	{
		MSD_DBG_ERROR(("Input param MSD_ATU_INT_STATUS in Willow_gfdbGetViolationIntf is NULL. \n"));
		return MSD_BAD_PARAM;
	}
	msdMemSet((void*)atuIntStatus, 0, sizeof(MSD_ATU_INT_STATUS));
    atuIntStatus->atuIntCause.ageOutVio = ((atuInt.atuIntCause & WILLOW_MSD_ATU_AGE_OUT_VIOLATION) == 0) ? MSD_FALSE : MSD_TRUE;
    atuIntStatus->atuIntCause.memberVio = ((atuInt.atuIntCause & WILLOW_MSD_ATU_MEMBER_VIOLATION) == 0) ? MSD_FALSE : MSD_TRUE;
    atuIntStatus->atuIntCause.missVio = ((atuInt.atuIntCause & WILLOW_MSD_ATU_MISS_VIOLATION) == 0) ? MSD_FALSE : MSD_TRUE;
    atuIntStatus->atuIntCause.fullVio = ((atuInt.atuIntCause & WILLOW_MSD_ATU_FULL_VIOLATION) == 0) ? MSD_FALSE : MSD_TRUE;
...
)
{
    ...
    if (NULL == macAddr)
    {
        MSD_DBG_ERROR(("Input param MSD_ETHERADDR in Willow_gfdbFindAtuMacEntryIntf is NULL. \n"));
        return MSD_BAD_PARAM;
    }
    
    msdMemSet((void*)(&entry), 0, sizeof(WILLOW_MSD_ATU_ENTRY));
    entry.DBNum = (MSD_U16)fid;
	msdMemCpy(entry.macAddr.arEther, macAddr, 6);

	retVal = Willow_gfdbFindAtuMacEntry(dev, &entry, found);
	if (MSD_OK != retVal || MSD_FALSE == *found)
		return retVal;

    if (NULL == atuEntry)
    {
        MSD_DBG_ERROR(("Input param MSD_ATU_ENTRY in Willow_gfdbFindAtuMacEntryIntf is NULL. \n"));
        return MSD_BAD_PARAM;
    }
	msdMemSet((void*)atuEntry, 0, sizeof(MSD_ATU_ENTRY));
	atuEntry->fid = entry.DBNum;
...
)
{
    ...
    while (1 == temp)
    {
        status = Willow_gfdbGetAtuEntryNextIntf(dev, &entry);
