...
#if PROJECT == PROJECT_V31C || PROJECT == PROJECT_V3NA
        chConfig.mode.clockPolarity = SpiIf_ClockPolarity_idleLow;
        chConfig.mode.shiftClock =
                SpiIf_ShiftClock_shiftTransmitDataOnLeadingEdge;
        chConfig.mode.dataHeading = SpiIf_DataHeading_msbFirst;
        chConfig.mode.dataWidth = 8;
        chConfig.mode.csActiveLevel = Ifx_ActiveState_low;
        chConfig.mode.csLeadDelay = SpiIf_SlsoTiming_4;
        chConfig.mode.csTrailDelay = 2;
        chConfig.mode.csInactiveDelay = 2;
        chConfig.baudrate = config->base.maximumBaudrate;
#endif
        {
            Ifx_QSPI_BACON bacon;
            uint8          cs = 0; // not relevant for slave
