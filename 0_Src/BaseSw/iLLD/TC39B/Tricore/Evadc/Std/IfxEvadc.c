...
Ifx_EVADC_G_RES IfxEvadc_getResultBasedOnRequestSource(Ifx_EVADC *evadc, Ifx_EVADC_G *group, IfxEvadc_ChannelId channel, IfxEvadc_RequestSource sourceType)
{
    ...
    else
    {
        if ((group->CHCTR[channel].B.RESTGT == 1))
        {
            ...
            tmpResult.B.DRC    = evadc->GLOB.RES.B.GNR; //The bitfields are the same but interpretation is different. TODO- define a generic result register type.
...
uint32 IfxEvadc_initializeFAdcD(Ifx_EVADC *evadc, uint32 fAdcD)
{
    uint32 divD;
    uint32 result;
#ifdef IFX_CFG_AURIX_SCUCCU_USED
