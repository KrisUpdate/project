...
{
    SJA1105_LOW_LEVEL_DRV_DEBUG_PRINTF(("%s\r\n", __func__));
    uint8_t configId;
    uint8_t ret = 0;
    uint8_t switchId;
...
extern uint8_t SJA1105P_exposeAurix_initSwitch(void)
{
    SJA1105_LOW_LEVEL_DRV_DEBUG_PRINTF(("%s\r\n", __func__));
    uint8_t configId;
    uint8_t ret = 0;
    ...
    for (switchId = 0; switchId < SJA1105P_N_SWITCHES; switchId++)
    {
        ret += SJA1105P_exposeAurix_loadConfig((uint8_t) (configId + switchId), switchId);
        ret += SJA1105P_getConfigurationFlags(&configFlags, switchId);
        if (configFlags.configs == 0U)
        {  /* configuration invalid */
            ret += 1U;
...
extern uint8_t SJA1105P_loadsim_initSwitch(void)
{
    SJA1105_LOW_LEVEL_DRV_DEBUG_PRINTF(("%s\r\n", __func__));
    uint8_t configId;
    uint8_t ret = 0;
    ...
    for (switchId = 0; switchId < SJA1105P_N_SWITCHES; switchId++)
    {
        ret += SJA1105P_loadsim_loadConfig((uint8_t) (configId + switchId), switchId);
        ret += SJA1105P_getConfigurationFlags(&configFlags, switchId);
        if (configFlags.configs == 0U)
        {  /* configuration invalid */
            ret += 1U;
