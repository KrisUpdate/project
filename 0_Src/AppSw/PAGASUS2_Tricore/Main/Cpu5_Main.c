...
App_Cpu g_AppCpu5;
volatile uint32 cpu5_idle_counter=0;
static volatile boolean eventFlag_1ms_stm_5 = FALSE;
static volatile boolean eventFlag_10ms_stm_5 = FALSE;
static volatile boolean eventFlag_100ms_stm_5 = FALSE;
static volatile boolean eventFlag_1s_stm_5 = FALSE;
...
extern boolean GLOBAL_WC_PATTERN_CPU5;
...
static void STM5_Initial(void)
{
    ...
    stmCompareConfig.triggerPriority      = ISR_PRIORITY_STM5_TICK;
    stmCompareConfig.comparatorInterrupt  = IfxStm_ComparatorInterrupt_ir0; /*User must select the interrupt output */
    stmCompareConfig.ticks                = IFX_CFG_STM5_TICKS_PER_MS*10;    /* we start after 10ms */
...
IFX_INTERRUPT (UsrIsr_Stm_5, 5, ISR_PRIORITY_STM5_TICK);

void UsrIsr_Stm_5(void)
{
    /* Set next 1ms scheduler tick alarm */
    IfxStm_increaseCompare(&MODULE_STM5, IfxStm_Comparator_0, IFX_CFG_STM5_TICKS_PER_MS);
    ...
    eventFlag_1ms_stm_5 = TRUE;

     /* action to do each 10ms */
    if ((g_AppCpu5.TickCount_1ms % 10) == 0)
    {
        eventFlag_10ms_stm_5 = TRUE;
    }
    /* action to do each 100ms */
    if ((g_AppCpu5.TickCount_1ms % 100) == 0)
    {
        eventFlag_100ms_stm_5 = TRUE;
    }
    if ((g_AppCpu5.TickCount_1ms % 1000) == 0)
    {
        eventFlag_1s_stm_5 = TRUE;
    }
}
...
void core5_main(void)
{
    ...
    g_AppCpu5.info.pllFreq = IfxScuCcu_getPllFrequency();
    g_AppCpu5.info.cpuFreq = IfxScuCcu_getCpuFrequency(IfxCpu_getCoreId());
    g_AppCpu5.info.sysFreq = IfxScuCcu_getSpbFrequency();
    g_AppCpu5.info.stmFreq = IfxStm_getFrequency(&MODULE_STM5);
    g_AppCpu5.TickCount_1ms = 0;
...
#if USE_WC_PATTERN == 1
            if(GLOBAL_RUN_WC_PATTERN_TEST && GLOBAL_WC_PATTERN_CPU5)
            {
                wc_start_trigger(IfxCpu_getCoreId(), IfxStm_getFrequency(&MODULE_STM5), Get_WC_Pattern_Time());
                GLOBAL_WC_PATTERN_CPU5 = FALSE;
            }
#endif
            cpu5_idle_counter++;
...
/*
        if(check_core_shell_enabled(CORE5)){
            Ifx_Shell_process(&g_AsclinShellInterface.shell);
        }
*/
            if (eventFlag_1ms_stm_5)
            {
                core_raise_count( (IfxCpu_getCoreId()-1));
                eventFlag_1ms_stm_5 = FALSE;
            }
            if (eventFlag_10ms_stm_5)
            {
            //just Core5(high priority) and Core4(low priority) implement emergency UART console
/*
            roll_call_cores_status_check_all();
            if(roll_call_cores_status_check(CORE0) == HANG_DECTION){
                core_shell_enable(CORE5);
            }
*/
                eventFlag_10ms_stm_5 = FALSE;
            }
            if (eventFlag_100ms_stm_5)
            {
                eventFlag_100ms_stm_5 = FALSE;
            }
            if (eventFlag_1s_stm_5)
            {
                eventFlag_1s_stm_5 = FALSE;
            }
        }
    }
}
