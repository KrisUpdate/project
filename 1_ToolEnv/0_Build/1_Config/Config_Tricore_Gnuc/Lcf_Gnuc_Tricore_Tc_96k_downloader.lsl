...
LCF_INTVEC0_START = 0x80D80000;
LCF_INTVEC1_START = 0x80D82000;
LCF_INTVEC2_START = 0x80D84000;
LCF_INTVEC3_START = 0x80D86000;
LCF_INTVEC4_START = 0x80D88000;
LCF_INTVEC5_START = 0x80D8A000;
...
UPDATE_FW_START = 0x80D00000;
UPDATE_FW_START_INTERFACE_CONST = 0x80D00020;

LCF_TRAPVEC0_START = 0x80D8C000;
LCF_TRAPVEC1_START = 0x80D8E000;
LCF_TRAPVEC2_START = 0x80D90000;
LCF_TRAPVEC3_START = 0x80D92000;
LCF_TRAPVEC4_START = 0x80D94000;
LCF_TRAPVEC5_START = 0x80D96000;
...
MEMORY
{
	...
	/* only use 0x80d00000 for pflsh 4, 2M */
    pfls4 (rx!p): org = 0x80d00000, len = 2M
    pfls4_nc (rx!p): org = 0xa0d00000, len = 2M
...
/*Sections located at absolute fixed address*/
    ...
    SECTIONS
    {
        .start_tc0 (LCF_STARTPTR_CPU0) : FLAGS(rxl) { KEEP (*(.start)); } > pfls4
        .interface_const (UPDATE_FW_START_INTERFACE_CONST) : { __IF_CONST = .; KEEP (*(.interface_const)); } > pfls4
    ...
    SECTIONS
    {
        .traptab_tc0 (LCF_TRAPVEC0_START) : { PROVIDE(__TRAPTAB_CPU0 = .); KEEP (*(.traptab_cpu0)); } > pfls4
        .traptab_tc1 (LCF_TRAPVEC1_START) : { PROVIDE(__TRAPTAB_CPU1 = .); KEEP (*(.traptab_cpu1)); } > pfls4
        .traptab_tc2 (LCF_TRAPVEC2_START) : { PROVIDE(__TRAPTAB_CPU2 = .); KEEP (*(.traptab_cpu2)); } > pfls4
        .traptab_tc3 (LCF_TRAPVEC3_START) : { PROVIDE(__TRAPTAB_CPU3 = .); KEEP (*(.traptab_cpu3)); } > pfls4
        .traptab_tc4 (LCF_TRAPVEC4_START) : { PROVIDE(__TRAPTAB_CPU4 = .); KEEP (*(.traptab_cpu4)); } > pfls4
        .traptab_tc5 (LCF_TRAPVEC5_START) : { PROVIDE(__TRAPTAB_CPU5 = .); KEEP (*(.traptab_cpu5)); } > pfls4
    }
    ...
    SECTIONS
    {
        .start_tc1 (LCF_STARTPTR_CPU1) : FLAGS(rxl) { KEEP (*(.start_cpu1)); } > pfls4
        .start_tc2 (LCF_STARTPTR_CPU2) : FLAGS(rxl) { KEEP (*(.start_cpu2)); } > pfls4
        .start_tc3 (LCF_STARTPTR_CPU3) : FLAGS(rxl) { KEEP (*(.start_cpu3)); } > pfls4
        .start_tc4 (LCF_STARTPTR_CPU4) : FLAGS(rxl) { KEEP (*(.start_cpu4)); } > pfls4
        .start_tc5 (LCF_STARTPTR_CPU5) : FLAGS(rxl) { KEEP (*(.start_cpu5)); } > pfls4
...
/* NV: we change use pfls4 to for update f/w */
CORE_ID = CPU1;
SECTIONS
{
    CORE_SEC(.rodata) : FLAGS(arl)
...
SECTIONS
{
	...
	/*This section is always required as Fw version is address absolutely restricted at address 0x80E00000*/
	CORE_ID = GLOBAL;
	SECTIONS
	{
		.upd_fw_version_data (0x80E00000) : FLAGS(arl)
		{
