...
LCF_INTVEC0_START = 0x80B80000;
LCF_INTVEC1_START = 0x80B82000;
LCF_INTVEC2_START = 0x80B84000;
LCF_INTVEC3_START = 0x80B86000;
LCF_INTVEC4_START = 0x80B88000;
LCF_INTVEC5_START = 0x80B8A000;
...
UPDATE_FW_START = 0x80B00000;
UPDATE_FW_START_INTERFACE_CONST = 0x80B00020;

LCF_TRAPVEC0_START = 0x80B8C000;
LCF_TRAPVEC1_START = 0x80B8E000;
LCF_TRAPVEC2_START = 0x80B90000;
LCF_TRAPVEC3_START = 0x80B92000;
LCF_TRAPVEC4_START = 0x80B94000;
LCF_TRAPVEC5_START = 0x80B96000;

LCF_STARTPTR_CPU0 = UPDATE_FW_START;
LCF_STARTPTR_CPU1 = UPDATE_FW_START + 0x1000;
LCF_STARTPTR_CPU2 = UPDATE_FW_START + 0x2000;
LCF_STARTPTR_CPU3 = UPDATE_FW_START + 0x3000;
LCF_STARTPTR_CPU4 = UPDATE_FW_START + 0x4000;
LCF_STARTPTR_CPU5 = UPDATE_FW_START + 0x5000;;
...
MEMORY
{
	...
	pfls3 (rx!p): org = 0x80800000, len = 2M
    pfls3_nc (rx!p): org = 0xa0800000, len = 2M
    
    pfls4 (rx!p): org = 0x80a00000, len = 1M
    pfls4_nc (rx!p): org = 0xa0a00000, len = 1M
...
/*Sections located at absolute fixed address*/
    ...
    SECTIONS
    {
        .start_tc0 (LCF_STARTPTR_CPU0) : FLAGS(rxl) { KEEP (*(.start)); } > pfls6
        .interface_const (UPDATE_FW_START_INTERFACE_CONST) : { __IF_CONST = .; KEEP (*(.interface_const)); } > pfls6
    ...
    SECTIONS
    {
        .traptab_tc0 (LCF_TRAPVEC0_START) : { PROVIDE(__TRAPTAB_CPU0 = .); KEEP (*(.traptab_cpu0)); } > pfls6
        .traptab_tc1 (LCF_TRAPVEC1_START) : { PROVIDE(__TRAPTAB_CPU1 = .); KEEP (*(.traptab_cpu1)); } > pfls6
        .traptab_tc2 (LCF_TRAPVEC2_START) : { PROVIDE(__TRAPTAB_CPU2 = .); KEEP (*(.traptab_cpu2)); } > pfls6
        .traptab_tc3 (LCF_TRAPVEC3_START) : { PROVIDE(__TRAPTAB_CPU3 = .); KEEP (*(.traptab_cpu3)); } > pfls6
        .traptab_tc4 (LCF_TRAPVEC4_START) : { PROVIDE(__TRAPTAB_CPU4 = .); KEEP (*(.traptab_cpu4)); } > pfls6
        .traptab_tc5 (LCF_TRAPVEC5_START) : { PROVIDE(__TRAPTAB_CPU5 = .); KEEP (*(.traptab_cpu5)); } > pfls6
    }
    ...
    SECTIONS
    {
        .start_tc1 (LCF_STARTPTR_CPU1) : FLAGS(rxl) { KEEP (*(.start_cpu1)); } > pfls6
        .start_tc2 (LCF_STARTPTR_CPU2) : FLAGS(rxl) { KEEP (*(.start_cpu2)); } > pfls6
        .start_tc3 (LCF_STARTPTR_CPU3) : FLAGS(rxl) { KEEP (*(.start_cpu3)); } > pfls6
        .start_tc4 (LCF_STARTPTR_CPU4) : FLAGS(rxl) { KEEP (*(.start_cpu4)); } > pfls6
        .start_tc5 (LCF_STARTPTR_CPU5) : FLAGS(rxl) { KEEP (*(.start_cpu5)); } > pfls6
...
/* NV: we change use pfls6 to for update f/w */
CORE_ID = CPU1;
SECTIONS
{
    CORE_SEC(.rodata) : FLAGS(arl)
...
SECTIONS
{
	...
	/*This section is always required as Fw version is address absolutely restricted at address 0x80C7FFF8*/
	CORE_ID = GLOBAL;
	SECTIONS
	{
		.upd_fw_version_data (0x80C7FFF8) : FLAGS(arl)
		{
			KEEP (*(.upd_fw_version_data));
		} > pfls7
	}
