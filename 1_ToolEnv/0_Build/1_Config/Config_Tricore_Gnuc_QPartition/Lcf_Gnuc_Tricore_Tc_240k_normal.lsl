...
LCF_INTVEC1_START = 0x80C06000;
LCF_INTVEC2_START = 0x80C08000;
LCF_INTVEC3_START = 0x80C0a000;
LCF_INTVEC4_START = 0x80C0c000;
LCF_INTVEC5_START = 0x80C0e000;
...
LCF_TRAPVEC0_START = 0x80c10000;
LCF_TRAPVEC1_START = 0x80c12000;
LCF_TRAPVEC2_START = 0x80c14000;
LCF_TRAPVEC3_START = 0x80c16000;
LCF_TRAPVEC4_START = 0x80c18000;
LCF_TRAPVEC5_START = 0x80c1a000;

LCF_STARTPTR_CPU0 = 0x80000000;
LCF_STARTPTR_CPU1 = 0x80c1c000;
LCF_STARTPTR_CPU2 = 0x80c1d000;
LCF_STARTPTR_CPU3 = 0x80c1e000;
LCF_STARTPTR_CPU4 = 0x80c20000;
LCF_STARTPTR_CPU5 = 0x80c21000;
...
MEMORY
{
	...
	pfls2 (rx!p): org = 0x80600000, len = 2M
    pfls2_nc (rx!p): org = 0xa0600000, len = 2M
	
	pfls3 (rx!p): org = 0x80800000, len = 1M
    pfls3_nc (rx!p): org = 0xa0800000, len = 1M
    
    pfls4 (rx!p): org = 0x80900000, len = 1M
    pfls4_nc (rx!p): org = 0xa0900000, len = 1M
	
    pfls5 (rx!p): org = 0x80a00000, len = 1M
    pfls5_nc (rx!p): org = 0xa0a00000, len = 1M	
    
    pfls6 (rx!p): org = 0x80b00000, len = 1M
    pfls6_nc (rx!p): org = 0xa0b00000, len = 1M		

	pfls7 (rx!p): org = 0x80c00000, len = 512K
    pfls7_nc (rx!p): org = 0xa0c00000, len = 512K		

	/* Added for ABMHD */
	pfls7_abmhd0 (r!p): org = 0x80c04000, len = 32
	pfls7_abmhd0_nc (r!p): org = 0xa0c04000, len = 32
	
	pfls7_abmhd_check_addr (r!p): org = 0x80c00000, len = 32
	pfls7_abmhd_check_addr_nc (r!p): org = 0xa0c00000, len = 32	

	pfls8 (rx!p): org = 0x80c80000, len = 512K
    pfls8_nc (rx!p): org = 0xa0c80000, len = 512K
	
	pfls9 (rx!p): org = 0x80d00000, len = 3M
    pfls9_nc (rx!p): org = 0xa0d00000, len = 3M		
...
REGION_MIRROR("pfls7", "pfls7_nc")
REGION_MIRROR("pfls8", "pfls8_nc")
REGION_MIRROR("pfls9", "pfls9_nc")
...
/*Sections located at absolute fixed address*/
	...
	/* Fixed memory Allocations for abmhd */
	SECTIONS
	{		
		.abmhd_0 (0x80c04000) : FLAGS(arl)
		{
			BootModeHeader0 = .;
			KEEP (*(.abmhd_0))
		} > pfls7_abmhd0
	...
	/* Fixed memory Allocations for abmhd check address */
	SECTIONS
	{		
		.abmhd_check_address_pfls7 (0x80c00000) : FLAGS(arl)
		{
			BootModeHeader0 = .;
			KEEP (*(.abmhd_check_address_pfls7))
		} > pfls7_abmhd_check_addr		
	}
    ...
    SECTIONS
    {
        .traptab_tc0 (LCF_TRAPVEC0_START) : { PROVIDE(__TRAPTAB_CPU0 = .); KEEP (*(.traptab_cpu0)); } > pfls7
        .traptab_tc1 (LCF_TRAPVEC1_START) : { PROVIDE(__TRAPTAB_CPU1 = .); KEEP (*(.traptab_cpu1)); } > pfls7
        .traptab_tc2 (LCF_TRAPVEC2_START) : { PROVIDE(__TRAPTAB_CPU2 = .); KEEP (*(.traptab_cpu2)); } > pfls7
        .traptab_tc3 (LCF_TRAPVEC3_START) : { PROVIDE(__TRAPTAB_CPU3 = .); KEEP (*(.traptab_cpu3)); } > pfls7
        .traptab_tc4 (LCF_TRAPVEC4_START) : { PROVIDE(__TRAPTAB_CPU4 = .); KEEP (*(.traptab_cpu4)); } > pfls7
        .traptab_tc5 (LCF_TRAPVEC5_START) : { PROVIDE(__TRAPTAB_CPU5 = .); KEEP (*(.traptab_cpu5)); } > pfls7
    }
    ...
    SECTIONS
    {
        .start_tc1 (LCF_STARTPTR_CPU1) : FLAGS(rxl) { KEEP (*(.start_cpu1)); } > pfls7
        .start_tc2 (LCF_STARTPTR_CPU2) : FLAGS(rxl) { KEEP (*(.start_cpu2)); } > pfls7
        .start_tc3 (LCF_STARTPTR_CPU3) : FLAGS(rxl) { KEEP (*(.start_cpu3)); } > pfls7
        .start_tc4 (LCF_STARTPTR_CPU4) : FLAGS(rxl) { KEEP (*(.start_cpu4)); } > pfls7
        .start_tc5 (LCF_STARTPTR_CPU5) : FLAGS(rxl) { KEEP (*(.start_cpu5)); } > pfls7
	...
	/*This section defines IFW firmware version at a fixed address 0x80C7FFF0*/
	CORE_ID = GLOBAL;
	SECTIONS
	{
		.fw_version_data (0x80C7FFF0) : FLAGS(arl)
		{
			KEEP (*(.fw_version_data));
		} > pfls7
	}
...
/* NV: we change all other CPU to use pfls1 to avoid conflict with update f/w */
CORE_ID = CPU1;
SECTIONS
{
    CORE_SEC(.rodata) : FLAGS(arl)
...
/* NV: we change all other CPU to use pfls1 to avoid conflic with update f/w */
CORE_ID = CPU1;
SECTIONS
{
    CORE_SEC(.text) : FLAGS(axl)
